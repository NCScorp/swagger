				...
				return {
											...
											validate : function () {
												if (this.isEnabled()) {
													if (!this.getValue()) {
														...
														videoMatch = ytVidId(this.getValue(), VIDEOTYPE);

														video = videoMatch[0];
														videoType = videoMatch[1];
					...
					onOk: function()
					{
						...
						if (this.getContentElement('youtubePlugin', 'txtEmbed').isEnabled()) {
							...
							var url = 'https://', params = [], startSecs, paramAutoplay='', isYoutube = true;
							...
							if (this.getContentElement('youtubePlugin', 'chkNoEmbed').getValue() === true) {
								...
								if (isYoutube) {
									...
									content += '<iframe allow="autoplay; fullscreen;" width="' + width + '" height="' + height + '" src="' + url + '" ' + responsiveStyle;
									content += 'frameborder="0" allowfullscreen></iframe>';
								}
							}
...
function handleLinkChange(el, api) {
	var video = ytVidId(el.getValue(), { youtube: 'YOUTUBE', vimeo: 'VIMEO' })[0];
 ...
 */
function ytVidId(url, videoTypes) {
	var p = /^(?:https?:\/\/)?(?:www\.)?(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=))((\w|-){11})(?:\S+)?$/;

	var vimeo = /^(http:\/\/|https:\/\/)?(?:www.)?(vimeo).com\/(\d{9})|(http:\/\/|https:\/\/)player.vimeo.com\/video\/(\d{9})/;
	
	var mathYotube = url.match(p);
	var mathVimeo = url.match(vimeo);

	if (mathYotube) {
		return [RegExp.$1, videoTypes.youtube];

	} else if (mathVimeo) {

		return mathVimeo[3] ? [mathVimeo[3], videoTypes.vimeo] : [mathVimeo[5], videoTypes.vimeo];
	}
