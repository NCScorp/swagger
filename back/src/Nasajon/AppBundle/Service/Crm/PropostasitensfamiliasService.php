...
use LogicException;
use Nasajon\MDABundle\Service\Crm\PropostasitensfamiliasService as ParentService;

/**
 * PropostasitensfamiliasService
 * Sobrescrito metodo insert para impedir a criação de um propostaitemfamilia caso orçamentos do propostaitem não estejam abertos.
 * Sobrescrito metodo delete para deletar o orçamento relacionado com o propostaitemfamilia caso orçamentos do propostaitem não estejam abertos. Caso contrário, não exclui o propostaitemfamilia.
 */
class PropostasitensfamiliasService extends ParentService
{
    /**
     * @var \Nasajon\MDABundle\Repository\Crm\PropostasitensfamiliasRepository
     */
    ...
    public function __construct(\Nasajon\MDABundle\Repository\Crm\PropostasitensfamiliasRepository $repository, $crmOrcamentosService, $responsabilidadesfinanceirasvaloresRepository)
    {
    ...
    /**
     * Verifica se há orçamentos associados a proposta item pai da proposta item familia. Se houver, significa que o fornecedor está acionado
     *
    ...
    public function findAll($tenant, $propostaitem, $id_grupoempresarial, Filter $filter = null){
        ...
        $familias = parent::findAll($tenant, $propostaitem, $id_grupoempresarial, $filter);
        return $familias;
    }
     ...
     * @param \Nasajon\MDABundle\Entity\Crm\Propostasitensfamilias $entity
     * @return string
     * @throws \Exception
     */
    public function insert($propostaitem, $logged_user, $tenant, $id_grupoempresarial, \Nasajon\MDABundle\Entity\Crm\Propostasitensfamilias $entity)
    {

        try{
            //não permite inserir se orcamento status != 0
            if (!$this->orcamentosAbertos($tenant, $id_grupoempresarial, $propostaitem)) {
                throw new \LogicException("Não é possível atribuir um produto à um item com orçamento em negociação/finalizado.", 1);
            }
            ...
            throw new \LogicException("Não foi possível adicionar o produto. " . $e->getMessage(), 1);
        }
     ...
     * @throws \Exception
     */
    public function delete($tenant, $id_grupoempresarial, $logged_user, \Nasajon\MDABundle\Entity\Crm\Propostasitensfamilias $entity)
    {
        try {
            $this->getRepository()->begin();

            if($this->possuiContrato($entity)){
                throw new \LogicException("Não é possível excluir um produto vinculado a um contrato.", 1);
            }
            ...
            throw new \LogicException("Não foi possível excluir o produto. " . $e->getMessage(), 1);
        }
    }

    /**
     * Verifica se o propostaitemfamilia possuem contrato.
     * @param \Nasajon\MDABundle\Entity\Crm\Propostasitensfamilias $entity
     * @return boolean
     */
     ...
     * @throws \Exception
     */
    public function update($propostaitem, $tenant, $id_grupoempresarial, $logged_user, \Nasajon\MDABundle\Entity\Crm\Propostasitensfamilias $entity){
        ...
        try{
            ...
            throw new \LogicException("Não foi possível editar o produto. " . $e->getMessage(), 1);
        }
    ...
    /**
     * Encontra todas as familias de propostasitens de uma proposta
     */
    public function getFamiliasProposta($tenant, $grupoempresarial, $proposta)
    {
        $familias = $this->getRepository()->getFamiliasProposta($tenant, $grupoempresarial, $proposta);
        return $familias;
    }
