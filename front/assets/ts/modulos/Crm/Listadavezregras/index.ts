...
export class CrmListadavezregrasListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'CrmListadavezregras', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'listadavezregra') {
        ...
        this.fields = $rootScope.FIELDS_CrmListadavezregras;
                $scope.$on('crm_listadavezregras_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_listadavezregras_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
