...
export class CrmMalotesdocumentosListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'malotedocumento') {
        ...
        this.fields = $rootScope.FIELDS_CrmMalotesdocumentos;
                $scope.$on('crm_malotesdocumentos_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_malotesdocumentos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
