...
export class CrmNegocioscontatosListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'CrmNegocioscontatos', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'id') {
        ...
        this.fields = $rootScope.FIELDS_CrmNegocioscontatos;
                $scope.$on('crm_negocioscontatos_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_negocioscontatos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
