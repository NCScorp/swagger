...
export class CrmNegocioscontatos {
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_negocioscontatos_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    if (data.length > 0) {
                        ...
                        this.$rootScope.$broadcast('crm_negocioscontatos_list_finished', this.entities);
                                        this.loaded = true;
    ...
    delete($identifier: any, force: boolean) {
        ...
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_negocioscontatos_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_negocioscontatos_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_negocioscontatos_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        ...
        if (entity.id) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_negocioscontatos_put', { documento : entity.documento.documento,  'id': entity.id }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_negocioscontatos_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                ...
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_negocioscontatos_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_negocioscontatos_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(documento: any, identifier : string) {
                
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_negocioscontatos_get', { 'documento': documento,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_negocioscontatos_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
