...
export class CrmFornecedoresenvolvidosanexos {
    ...
    reload() {
        ...
        delete this.filters['fornecedorenvolvidoanexo'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_fornecedoresenvolvidosanexos_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
    ...
    load() {
        if (!this.loadParams.busy && !this.loadParams.finished && this.loadParams.to_load > 0  && !this.loaded ) {
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    if (data.length > 0) {
                        ...
                        this.$rootScope.$broadcast('crm_fornecedoresenvolvidosanexos_list_finished', this.entities);
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].fornecedorenvolvidoanexo === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.fornecedorenvolvidoanexo;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_fornecedoresenvolvidosanexos_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_fornecedoresenvolvidosanexos_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_fornecedoresenvolvidosanexos_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.fornecedorenvolvidoanexo) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_fornecedoresenvolvidosanexos_put', { fornecedorenvolvido : entity.fornecedorenvolvido,  'id': entity.fornecedorenvolvidoanexo }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_fornecedoresenvolvidosanexos_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.fornecedorenvolvidoanexo) {
                    entity.fornecedorenvolvidoanexo = response.data.fornecedorenvolvidoanexo;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_fornecedoresenvolvidosanexos_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_fornecedoresenvolvidosanexos_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(fornecedorenvolvido: any, identifier : string) {
                
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_fornecedoresenvolvidosanexos_get', { 'fornecedorenvolvido': fornecedorenvolvido,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_fornecedoresenvolvidosanexos_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
