...
export class CrmFornecedoresenvolvidosanexosListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'fornecedorenvolvidoanexo') {
        ...
        this.fields = $rootScope.FIELDS_CrmFornecedoresenvolvidosanexos;
                $scope.$on('crm_fornecedoresenvolvidosanexos_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_fornecedoresenvolvidosanexos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
