...
export class CrmComposicoesListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'composicao') {
        ...
        this.fields = $rootScope.FIELDS_CrmComposicoes;
                $scope.$on('crm_composicoes_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_composicoes_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
