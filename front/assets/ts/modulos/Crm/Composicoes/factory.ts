...
export class CrmComposicoes {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.composicao;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_composicoes_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_composicoes_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_composicoes_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.composicao) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_composicoes_put', {  'id': entity.composicao }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_composicoes_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.composicao) {
                    entity.composicao = response.data.composicao;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_composicoes_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_composicoes_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
