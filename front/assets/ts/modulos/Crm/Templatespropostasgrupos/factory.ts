...
export class CrmTemplatespropostasgrupos {
    ...
    reload() {
        ...
        delete this.filters['templatepropostagrupo'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_templatespropostasgrupos_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.templatepropostagrupo = this.entities[this.entities.length - 1].templatepropostagrupo;
                        this.after.templatepropostagrupo = this.entities[this.entities.length - 1].templatepropostagrupo;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_templatespropostasgrupos_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].templatepropostagrupo === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.templatepropostagrupo;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_templatespropostasgrupos_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_templatespropostasgrupos_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_templatespropostasgrupos_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.templatepropostagrupo) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_templatespropostasgrupos_put', { cliente : entity.cliente,  'id': entity.templatepropostagrupo }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_templatespropostasgrupos_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.templatepropostagrupo) {
                    entity.templatepropostagrupo = response.data.templatepropostagrupo;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_templatespropostasgrupos_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_templatespropostasgrupos_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(cliente: any, identifier : string) {
                
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_templatespropostasgrupos_get', { 'cliente': cliente,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_templatespropostasgrupos_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
