...
export class CrmMotivosdesqualificacoesprenegociosListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'CrmMotivosdesqualificacoesprenegocios', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'motivodesqualificacaoprenegocio') {
        ...
        this.fields = $rootScope.FIELDS_CrmMotivosdesqualificacoesprenegocios;
                $scope.$on('crm_motivosdesqualificacoesprenegocios_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_motivosdesqualificacoesprenegocios_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
