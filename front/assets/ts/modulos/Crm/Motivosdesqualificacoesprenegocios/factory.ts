...
export class CrmMotivosdesqualificacoesprenegocios {
    ...
    reload() {
        ...
        delete this.filters['motivodesqualificacaoprenegocio'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_motivosdesqualificacoesprenegocios_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    if (data.length > 0) {
                        ...
                        this.$rootScope.$broadcast('crm_motivosdesqualificacoesprenegocios_list_finished', this.entities);
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].motivodesqualificacaoprenegocio === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.motivodesqualificacaoprenegocio;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_motivosdesqualificacoesprenegocios_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_motivosdesqualificacoesprenegocios_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_motivosdesqualificacoesprenegocios_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.motivodesqualificacaoprenegocio) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_motivosdesqualificacoesprenegocios_put', {  'id': entity.motivodesqualificacaoprenegocio }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_motivosdesqualificacoesprenegocios_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.motivodesqualificacaoprenegocio) {
                    entity.motivodesqualificacaoprenegocio = response.data.motivodesqualificacaoprenegocio;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_motivosdesqualificacoesprenegocios_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_motivosdesqualificacoesprenegocios_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_motivosdesqualificacoesprenegocios_get', {  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_motivosdesqualificacoesprenegocios_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
