...
export class CrmFornecedorespedidoselecaoListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'CrmFornecedorespedidoselecao', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['municipioibge', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'fornecedor') {
        ...
        this.fields = $rootScope.FIELDS_CrmFornecedorespedidoselecao;
                $scope.$on('crm_fornecedorespedidoselecao_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_fornecedorespedidoselecao_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
