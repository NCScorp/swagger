...
export class CrmFornecedorespedidoselecao {
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_fornecedorespedidoselecao_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.municipioibge = this.entities[this.entities.length - 1].municipioibge;
                        this.after.fornecedor = this.entities[this.entities.length - 1].fornecedor;
                                            }
                                        if (data.length <= 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_fornecedorespedidoselecao_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    delete($identifier: any, force: boolean) {
        ...
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_fornecedorespedidoselecao_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_fornecedorespedidoselecao_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_fornecedorespedidoselecao_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        ...
        if (entity.fornecedor) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_fornecedorespedidoselecao_put', {  'id': entity.fornecedor }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_fornecedorespedidoselecao_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                ...
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_fornecedorespedidoselecao_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_fornecedorespedidoselecao_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_fornecedorespedidoselecao_get', {  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_fornecedorespedidoselecao_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
