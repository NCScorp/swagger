...
import { ModalExclusaoPropCapituloService } from './modal-exclusao';

export class CrmPropostascapitulos {
    static $inject = ['$http', 'nsjRouting', '$rootScope', '$q',
        'modalExclusaoPropCapituloService', 'toaster'];
    public entities: any[] = [];
    ...
    constructor(public $http: angular.IHttpService, public nsjRouting: any, public $rootScope: angular.IScope, public $q: angular.IQService,
        public ModalExclusaoPropCapituloService: ModalExclusaoPropCapituloService, public toaster: any) {
    }
    ...
    reload() {
        ...
        delete this.filters['propostacapitulo'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_propostascapitulos_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true, true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.propostacapitulo = this.entities[this.entities.length - 1].propostacapitulo;
                        this.after.propostacapitulo = this.entities[this.entities.length - 1].propostacapitulo;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_propostascapitulos_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].propostacapitulo === identifier) {
    ...
    // Abre modal de confirmação de exclusão da proposta capitulo (agrupador)
    abrirModalExclusao(propostacapitulo: any){

        let modal = this.ModalExclusaoPropCapituloService.open(propostacapitulo);
        modal.result.then((response: any) => {
            this.delete(propostacapitulo.id, true); 
        })
        .catch((error: any) => {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object' && $identifier != null) {
            $identifier = $identifier.propostacapitulo;
        }
        if ($identifier) {
            if (force) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_propostascapitulos_delete', angular.extend(this.constructors, {'id': $identifier }), true, true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_propostascapitulos_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_propostascapitulos_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.propostacapitulo) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_propostascapitulos_put', { proposta : entity.proposta,  'id': entity.propostacapitulo }, true, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_propostascapitulos_create', angular.extend(this.constructors), true, true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.propostacapitulo) {
                    entity.propostacapitulo = response.data.propostacapitulo;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_propostascapitulos_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_propostascapitulos_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(proposta: any, identifier : string) {
                
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_propostascapitulos_get', { 'proposta': proposta,  'id': identifier }, true, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_propostascapitulos_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
