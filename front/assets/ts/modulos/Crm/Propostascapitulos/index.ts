...
export class CrmPropostascapitulosListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'propostacapitulo') {
        ...
        this.fields = $rootScope.FIELDS_CrmPropostascapitulos;
                $scope.$on('crm_propostascapitulos_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_propostascapitulos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
