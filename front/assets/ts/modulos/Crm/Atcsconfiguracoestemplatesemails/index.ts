...
export class CrmAtcsconfiguracoestemplatesemailsListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'atcconfiguracaotemplateemail') {
        ...
        this.fields = $rootScope.FIELDS_CrmAtcsconfiguracoestemplatesemails;
                $scope.$on('crm_atcsconfiguracoestemplatesemails_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_atcsconfiguracoestemplatesemails_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
