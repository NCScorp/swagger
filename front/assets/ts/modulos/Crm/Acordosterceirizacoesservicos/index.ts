...
export class CrmAcordosterceirizacoesservicosListController {

    static $inject = ['utilService', '$scope', '$stateParams', '$state', 'CrmAcordosterceirizacoesservicos', 'toaster', '$rootScope', '$location', 'nsjRouting'];
    ...
    constructor(
        ...
        for (let i in $stateParams) {
            ...
            } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'acordoterceirizacaoservico') {
        ...
        this.fields = $rootScope.FIELDS_CrmAcordosterceirizacoesservicos;
        $scope.$on('crm_acordosterceirizacoesservicos_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_acordosterceirizacoesservicos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
