...
export class CrmTemplatescomposicoesfuncoes {
    ...
    reload() {
        ...
        delete this.filters['templatecomposicaofuncao'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_templatescomposicoesfuncoes_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.templatecomposicaofuncao = this.entities[this.entities.length - 1].templatecomposicaofuncao;
                        this.after.templatecomposicaofuncao = this.entities[this.entities.length - 1].templatecomposicaofuncao;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_templatescomposicoesfuncoes_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].templatecomposicaofuncao === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.templatecomposicaofuncao;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_templatescomposicoesfuncoes_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_templatescomposicoesfuncoes_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_templatescomposicoesfuncoes_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.templatecomposicaofuncao) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_templatescomposicoesfuncoes_put', { templatepropostacomposicao : entity.templatepropostacomposicao,  'id': entity.templatecomposicaofuncao }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_templatescomposicoesfuncoes_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.templatecomposicaofuncao) {
                    entity.templatecomposicaofuncao = response.data.templatecomposicaofuncao;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_templatescomposicoesfuncoes_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_templatescomposicoesfuncoes_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(templatepropostacomposicao: any, identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_templatescomposicoesfuncoes_get', { 'templatepropostacomposicao': templatepropostacomposicao,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_templatescomposicoesfuncoes_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
