...
export class CrmSegmentosatuacaoListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'segmentoatuacao') {
        ...
        this.fields = $rootScope.FIELDS_CrmSegmentosatuacao;
                $scope.$on('crm_segmentosatuacao_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_segmentosatuacao_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
