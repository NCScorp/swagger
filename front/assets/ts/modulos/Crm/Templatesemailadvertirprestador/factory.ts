...
export class CrmTemplatesemailadvertirprestador {
    ...
    reload() {
        ...
        delete this.filters['templateemailadvertirprestador'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_templatesemailadvertirprestador_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true, true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.templateemailadvertirprestador = this.entities[this.entities.length - 1].templateemailadvertirprestador;
                        this.after.templateemailadvertirprestador = this.entities[this.entities.length - 1].templateemailadvertirprestador;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_templatesemailadvertirprestador_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].templateemailadvertirprestador === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.templateemailadvertirprestador;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_templatesemailadvertirprestador_delete', angular.extend(this.constructors, {'id': $identifier }), true, true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_templatesemailadvertirprestador_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_templatesemailadvertirprestador_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.templateemailadvertirprestador) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_templatesemailadvertirprestador_put', {  'id': entity.templateemailadvertirprestador }, true, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_templatesemailadvertirprestador_create', angular.extend(this.constructors), true, true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.templateemailadvertirprestador) {
                    entity.templateemailadvertirprestador = response.data.templateemailadvertirprestador;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_templatesemailadvertirprestador_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_templatesemailadvertirprestador_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_templatesemailadvertirprestador_get', {  'id': identifier }, true, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_templatesemailadvertirprestador_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
