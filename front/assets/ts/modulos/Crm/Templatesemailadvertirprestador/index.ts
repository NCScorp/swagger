...
export class CrmTemplatesemailadvertirprestadorListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['estabelecimento', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'templateemailadvertirprestador') {
        ...
        this.fields = $rootScope.FIELDS_CrmTemplatesemailadvertirprestador;
                $scope.$on('crm_templatesemailadvertirprestador_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_templatesemailadvertirprestador_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
