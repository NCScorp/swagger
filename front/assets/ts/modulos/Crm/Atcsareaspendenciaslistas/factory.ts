...
export class CrmAtcsareaspendenciaslistas {
    ...
    reload() {
        ...
        delete this.filters['negocioareapendencialista'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_atcsareaspendenciaslistas_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.negocioareapendencialista = this.entities[this.entities.length - 1].negocioareapendencialista;
                        this.after.negocioareapendencialista = this.entities[this.entities.length - 1].negocioareapendencialista;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_atcsareaspendenciaslistas_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].negocioareapendencialista === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.negocioareapendencialista;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_atcsareaspendenciaslistas_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_atcsareaspendenciaslistas_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_atcsareaspendenciaslistas_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.negocioareapendencialista) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_atcsareaspendenciaslistas_put', { negocioarea : entity.negocioarea,  'id': entity.negocioareapendencialista }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_atcsareaspendenciaslistas_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.negocioareapendencialista) {
                    entity.negocioareapendencialista = response.data.negocioareapendencialista;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_atcsareaspendenciaslistas_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_atcsareaspendenciaslistas_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(negocioarea: any, identifier : string) {
                
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_atcsareaspendenciaslistas_get', { 'negocioarea': negocioarea,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_atcsareaspendenciaslistas_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
