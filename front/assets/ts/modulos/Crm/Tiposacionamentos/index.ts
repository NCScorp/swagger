...
export class CrmTiposacionamentosListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'tiposacionamento') {
        ...
        this.fields = $rootScope.FIELDS_CrmTiposacionamentos;
                $scope.$on('crm_tiposacionamentos_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_tiposacionamentos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
