...
export class CrmAtcspendenciaslistasListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'negociopendencialista') {
        ...
        this.fields = $rootScope.FIELDS_CrmAtcspendenciaslistas;
                $scope.$on('crm_atcspendenciaslistas_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_atcspendenciaslistas_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
