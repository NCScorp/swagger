...
import { ModalExclusaoPropItemService } from './modal-exclusao';

export class CrmPropostasitens {
    static $inject = ['$http', 'nsjRouting', '$rootScope', '$q', 
        'modalExclusaoPropItemService', 'toaster'];
    public entities: any[] = [];
    ...
    constructor(public $http: angular.IHttpService, public nsjRouting: any, public $rootScope: angular.IScope, public $q: angular.IQService,
        public ModalExclusaoPropItemService: ModalExclusaoPropItemService, public toaster: any) {
    }
    ...
    reload() {
        ...
        delete this.filters['propostaitem'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_propostasitens_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true, true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.propostaitem = this.entities[this.entities.length - 1].propostaitem;
                        this.after.propostaitem = this.entities[this.entities.length - 1].propostaitem;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_propostasitens_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].propostaitem === identifier) {
    ...
    // Abre modal de confirmação de exclusão da proposta item (serviço)
    abrirModalExclusao(propostaitem: any){

        let modal = this.ModalExclusaoPropItemService.open(propostaitem);
        modal.result.then((response: any) => {
            this.delete(propostaitem.id, true); 
        })
        .catch((error: any) => {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object' && $identifier != null) {
            $identifier = $identifier.propostaitem;
        }
        if ($identifier) {
            if (force) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_propostasitens_delete', angular.extend(this.constructors, {'id': $identifier }), true, true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_propostasitens_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_propostasitens_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.propostaitem) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_propostasitens_put', { negocio : entity.negocio.negocio, proposta : entity.proposta.proposta,  'id': entity.propostaitem }, true, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_propostasitens_create', angular.extend(this.constructors), true, true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.propostaitem) {
                    entity.propostaitem = response.data.propostaitem;
                }
                entity.$$_saved = true;
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_propostasitens_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('crm_propostasitens_propostas_itens_vincular_fornecedor', { negocio : entity.negocio.negocio, proposta : entity.proposta.proposta,  'id': entity.propostaitem }, true, true),
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('crm_propostasitens_propostas_itens_desvincular_fornecedor', { negocio : entity.negocio.negocio, proposta : entity.proposta.proposta,  'id': entity.propostaitem }, true, true),
            data: angular.copy(entity)
            })
            .then((response: any) => {
                resolve(response.data);
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('crm_propostasitens_propostasitensfornecedorescolhacliente', { negocio : entity.negocio.negocio, proposta : entity.proposta.proposta,  'id': entity.propostaitem }, true, true),
            data: angular.copy(entity)
            })
            .then((response: any) => {
                resolve(response.data);
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('crm_propostasitens_orcamentos_excluir', { negocio : entity.negocio.negocio, proposta : entity.proposta.proposta,  'id': entity.propostaitem }, true, true),
            data: angular.copy(entity)
            })
            .then((response: any) => {
                resolve(response.data);
    ...
    get(negocio: any, proposta: any, identifier : string) {
                
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_propostasitens_get', { 'negocio': negocio, 'proposta': proposta,  'id': identifier }, true, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_propostasitens_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
