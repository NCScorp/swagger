...
export class CrmPropostasitensListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['propostacapitulo', 'fornecedor', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'propostaitem') {
        ...
        this.fields = $rootScope.FIELDS_CrmPropostasitens;
                $scope.$on('crm_propostasitens_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_propostasitens_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
