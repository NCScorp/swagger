...
export class CrmAtcsconfiguracoesdocumentositensListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'atcconfiguracaodocumentoitem') {
        ...
        this.fields = $rootScope.FIELDS_CrmAtcsconfiguracoesdocumentositens;
                $scope.$on('crm_atcsconfiguracoesdocumentositens_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_atcsconfiguracoesdocumentositens_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
