...
export class CrmAtcclientefornecedorescontatos {
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_atcclientefornecedorescontatos_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.contato = this.entities[this.entities.length - 1].contato;
                        this.after.contato = this.entities[this.entities.length - 1].contato;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_atcclientefornecedorescontatos_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].contato === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.contato;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_atcclientefornecedorescontatos_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_atcclientefornecedorescontatos_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_atcclientefornecedorescontatos_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        ...
        if (entity.contato) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_atcclientefornecedorescontatos_put', { negocio : entity.negocio,  'id': entity.contato }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_atcclientefornecedorescontatos_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.contato) {
                    entity.contato = response.data.contato;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_atcclientefornecedorescontatos_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_atcclientefornecedorescontatos_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(negocio: any, identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_atcclientefornecedorescontatos_get', { 'negocio': negocio,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_atcclientefornecedorescontatos_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
