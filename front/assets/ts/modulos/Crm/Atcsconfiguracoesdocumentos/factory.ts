...
import { IAtcConfiguracaoDocumento } from './classes';

export class CrmAtcsconfiguracoesdocumentos {
    ...
    public getListaFromApi(): Promise<IAtcConfiguracaoDocumento[]> {
        return new Promise((resolve, reject) => {
            this._load(this.constructors, {}, '', {}, true).then((dados: any) => {
                resolve(dados);
            }).catch((error) => {
                reject(error);
            });
    ...
    /**
     * Busca a lista de templates de emails de documentos (que no caso é um array com apenas 1 item)
     */
    public getTemplatesEmail() {
        return new Promise(async (resolve, reject) => {
          await this.$http({
            method: 'GET',
            url: this.nsjRouting.generate('crm_atcs_configuracoes_documentos_retorna_template_email', {}, true, true),
            timeout: null
          })
            .then((response: any) => {
    ...
    reload() {
        ...
        delete this.filters['atcconfiguracaodocumento'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_atcs_configuracoes_documentos_index', angular.extend({}, constructors, { 'offset': offset, 'filter': search }, filters ? filters : this.filters), true, true),
        ...
        if (!this.loadParams.busy && !this.loadParams.finished && this.loadParams.to_load > 0) {
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    ...
                    this.$rootScope.$broadcast('crm_atcsconfiguracoesdocumentos_list_finished', this.entities);
                    this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].atcconfiguracaodocumento === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof ($identifier) === 'object') {
            $identifier = $identifier.atcconfiguracaodocumento;
        }
        if ($identifier) {
            if (force) {
                this.loading_deferred = this.$q.defer();
                this.$http
                    .delete(this.nsjRouting.generate('crm_atcs_configuracoes_documentos_delete', angular.extend(this.constructors, { 'id': $identifier }), true, true))
                    .then((response: any) => {
                        this.$rootScope.$broadcast('crm_atcsconfiguracoesdocumentos_deleted', {
                            entity: this.entity,
                            response: response
                        });
                    })
                    .catch((response: any) => {
                        this.$rootScope.$broadcast('crm_atcsconfiguracoesdocumentos_delete_error', {
                            entity: this.entity,
                            response: response
                        });
                    });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.atcconfiguracaodocumento) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_atcs_configuracoes_documentos_put', { 'id': entity.atcconfiguracaodocumento }, true, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_atcs_configuracoes_documentos_create', angular.extend(this.constructors), true, true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.atcconfiguracaodocumento) {
                    entity.atcconfiguracaodocumento = response.data.atcconfiguracaodocumento;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_atcsconfiguracoesdocumentos_submitted', {
                    entity: entity,
                    response: response,
                    autosave: autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_atcsconfiguracoesdocumentos_submit_error', {
                    entity: entity,
                    response: response,
                    autosave: autosave
                });
    ...
    get(identifier: string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
                .get(this.nsjRouting.generate('crm_atcs_configuracoes_documentos_get', { 'id': identifier }, true, true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_atcsconfiguracoesdocumentos_loaded', response.data);
                    resolve(response.data);
                })
                .catch((response: any) => {
                    reject(response);
