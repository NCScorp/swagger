...
export class CrmAtcsconfiguracoesdocumentosListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'atcconfiguracaodocumento') {
        ...
        this.fields = $rootScope.FIELDS_CrmAtcsconfiguracoesdocumentos;
                $scope.$on('crm_atcsconfiguracoesdocumentos_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_atcsconfiguracoesdocumentos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
