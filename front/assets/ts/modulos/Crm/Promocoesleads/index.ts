...
export class CrmPromocoesleadsListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['bloqueado', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'promocaolead') {
        ...
        this.fields = $rootScope.FIELDS_CrmPromocoesleads;
                $scope.$on('crm_promocoesleads_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_promocoesleads_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
