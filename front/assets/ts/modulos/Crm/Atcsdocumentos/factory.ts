...
import { CrmAtcDocumento } from './classes/crmatcdocumento';
import { ICrmAtcDocumento } from './classes/icrmatcdocumento';

export class CrmAtcsdocumentos {
    ...
    reload() {
        ...
        delete this.filters['negociodocumento'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_atcsdocumentos_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.negociodocumento = this.entities[this.entities.length - 1].negociodocumento;
                        this.after.negociodocumento = this.entities[this.entities.length - 1].negociodocumento;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_atcsdocumentos_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].negociodocumento === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.negociodocumento;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_atcsdocumentos_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_atcsdocumentos_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_atcsdocumentos_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.negociodocumento) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_atcsdocumentos_put', {  'id': entity.negociodocumento }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_atcsdocumentos_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.negociodocumento) {
                    entity.negociodocumento = response.data.negociodocumento;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_atcsdocumentos_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_atcsdocumentos_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    recebeDocumento(entity: any) {     
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('crm_atcsdocumentos_recebe_documento', {  'id': entity.negociodocumento }, true),
    ...
    preAnalise(entity: any) {     
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('crm_atcsdocumentos_pre_analise', {  'id': entity.negociodocumento }, true),
            data: angular.copy(entity)
            })
            .then((response: any) => {
                resolve(response.data);
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_atcsdocumentos_get', {  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_atcsdocumentos_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
    ...
    /**
     * Baixa um documento do atendimento, retornando o arquivo binário
     * @param id
     */
    baixarDocumento(id: string): Promise<any>{
        ...
        return new Promise((resolve, reject) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_atcsdocumentos_baixardocumento', angular.extend(this.constructors, {id: id}), true),
                responseType: "blob",
            }).then((response: any) => {
                resolve(response.data);
            }).catch((response: any) => {
     ...
     * @param entity 
     */
    public create(entity: ICrmAtcDocumento): Promise<ICrmAtcDocumento> {
        return new Promise((resolve, reject) => {
    ...
    /**
     * Busca lista de todos os documentos enviados no atendimento
     * @param atc 
     */
    public getAll(filter: any = {}): Promise<CrmAtcDocumento[]> {
        if (this.constructors == null) {
            this.constructors = {};
        }
        ...
        return new Promise((resolve, reject) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_atcsdocumentos_index', filter, true)
            }).then((response: any) => {
                const arrDados = CrmAtcDocumento.getFromListaApi(response.data);
                resolve(arrDados);
                this.$rootScope.$broadcast('crm_atcsdocumentos_list_finished', arrDados);
            }).catch((erro: any) => {
                reject(erro);
            })
        });
    ...
    /**
     * Exclui um documento do atendimento
     * @param id
     */
    public excluir(id: string): Promise<any>{
        ...
        return new Promise((resolve, reject) => {
            this.$http.delete(
                this.nsjRouting.generate('crm_atcsdocumentos_delete', angular.extend(this.constructors, {id: id}), true)
            ).then((response: any) => {
                this.$rootScope.$broadcast('crm_atcsdocumentos_deleted', {
                    entity: this.entity,
                    response : response
