...
export class CrmResponsabilidadesfinanceirasvalores {
    ...
    reload() {
        ...
        delete this.filters['responsabilidadefinanceiravalor'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_responsabilidadesfinanceirasvalores_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.responsabilidadefinanceiravalor = this.entities[this.entities.length - 1].responsabilidadefinanceiravalor;
                        this.after.responsabilidadefinanceiravalor = this.entities[this.entities.length - 1].responsabilidadefinanceiravalor;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_responsabilidadesfinanceirasvalores_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].responsabilidadefinanceiravalor === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.responsabilidadefinanceiravalor;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_responsabilidadesfinanceirasvalores_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_responsabilidadesfinanceirasvalores_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_responsabilidadesfinanceirasvalores_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.responsabilidadefinanceiravalor) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_responsabilidadesfinanceirasvalores_put', { responsabilidadefinanceira : entity.responsabilidadefinanceira,  'id': entity.responsabilidadefinanceiravalor }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_responsabilidadesfinanceirasvalores_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.responsabilidadefinanceiravalor) {
                    entity.responsabilidadefinanceiravalor = response.data.responsabilidadefinanceiravalor;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_responsabilidadesfinanceirasvalores_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_responsabilidadesfinanceirasvalores_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('crm_responsabilidadesfinanceirasvalores_vincular_itens_de_contrato', { responsabilidadefinanceira : entity.responsabilidadefinanceira,  'id': entity.responsabilidadefinanceiravalor }, true),
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('crm_responsabilidadesfinanceirasvalores_desvincular_itens_de_contrato', { responsabilidadefinanceira : entity.responsabilidadefinanceira,  'id': entity.responsabilidadefinanceiravalor }, true),
            data: angular.copy(entity)
            })
            .then((response: any) => {
                resolve(response.data);
    ...
    get(responsabilidadefinanceira: any, identifier : string) {
                
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_responsabilidadesfinanceirasvalores_get', { 'responsabilidadefinanceira': responsabilidadefinanceira,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_responsabilidadesfinanceirasvalores_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
