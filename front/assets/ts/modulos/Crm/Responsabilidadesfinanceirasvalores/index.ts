...
export class CrmResponsabilidadesfinanceirasvaloresListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['responsavelfinanceiro', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'responsabilidadefinanceiravalor') {
        ...
        this.fields = $rootScope.FIELDS_CrmResponsabilidadesfinanceirasvalores;
                $scope.$on('crm_responsabilidadesfinanceirasvalores_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_responsabilidadesfinanceirasvalores_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
