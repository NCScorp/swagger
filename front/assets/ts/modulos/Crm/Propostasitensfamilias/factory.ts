...
import { ModalExclusaoPropItemFamiliaService } from './modal-exclusao';

export class CrmPropostasitensfamilias {
    static $inject = ['$http', 'nsjRouting', '$rootScope', '$q',
        'modalExclusaoPropItemFamiliaService', 'toaster'];
    public entities: any[] = [];
    ...
    constructor(public $http: angular.IHttpService, public nsjRouting: any, public $rootScope: angular.IScope, public $q: angular.IQService,
        public ModalExclusaoPropItemFamiliaService: ModalExclusaoPropItemFamiliaService, public toaster: any) {
    }
    ...
    reload() {
        ...
        delete this.filters['propostaitemfamilia'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_propostasitens_familias_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true, true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.propostaitemfamilia = this.entities[this.entities.length - 1].propostaitemfamilia;
                        this.after.propostaitemfamilia = this.entities[this.entities.length - 1].propostaitemfamilia;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_propostasitensfamilias_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].propostaitemfamilia === identifier) {
    ...
    // Abre modal de confirmação de exclusão da proposta item familia (produto)
    abrirModalExclusao(propostaitemfamilia: any){

        let modal = this.ModalExclusaoPropItemFamiliaService.open(propostaitemfamilia);
        modal.result.then((response: any) => {
            this.delete(propostaitemfamilia, true);
        })
        .catch((error: any) => {
    ...
    delete($parameters: any, force: boolean) {
        if ($parameters.id) {
            if (force) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_propostasitens_familias_delete', angular.extend(this.constructors, {'id': $parameters.id, 'negocio': $parameters.negocio, 'proposta': $parameters.proposta }), true, true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_propostasitensfamilias_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_propostasitensfamilias_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.propostaitemfamilia) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_propostasitens_familias_put', { propostaitem : entity.propostaitem,  'id': entity.propostaitemfamilia, 'negocio': entity.negocio, 'proposta': entity.proposta }, true, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_propostasitens_familias_create', angular.extend(this.constructors, {'negocio': entity.negocio, 'proposta': entity.proposta}), true, true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.propostaitemfamilia) {
                    entity.propostaitemfamilia = response.data.propostaitemfamilia;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_propostasitensfamilias_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_propostasitensfamilias_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(propostaitem: any, identifier : string) {
                
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_propostasitens_familias_get', { 'propostaitem': propostaitem,  'id': identifier }, true, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_propostasitensfamilias_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
