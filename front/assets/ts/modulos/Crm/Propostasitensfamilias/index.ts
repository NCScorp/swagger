...
export class CrmPropostasitensfamiliasListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'propostaitemfamilia') {
        ...
        this.fields = $rootScope.FIELDS_CrmPropostasitensfamilias;
                $scope.$on('crm_propostasitensfamilias_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_propostasitensfamilias_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
