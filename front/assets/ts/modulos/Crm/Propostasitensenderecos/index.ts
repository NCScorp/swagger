...
export class CrmPropostasitensenderecosListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'propostaitemendereco') {
        ...
        this.fields = $rootScope.FIELDS_CrmPropostasitensenderecos;
                $scope.$on('crm_propostasitensenderecos_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_propostasitensenderecos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
