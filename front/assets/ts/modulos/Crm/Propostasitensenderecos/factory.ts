...
export class CrmPropostasitensenderecos {
    ...
    reload() {
        ...
        delete this.filters['propostaitemendereco'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_propostasitensenderecos_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true, this.ERP_API),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.propostaitemendereco = this.entities[this.entities.length - 1].propostaitemendereco;
                        this.after.propostaitemendereco = this.entities[this.entities.length - 1].propostaitemendereco;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_propostasitensenderecos_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].propostaitemendereco === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.propostaitemendereco;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_propostasitensenderecos_delete', angular.extend(this.constructors, {'id': $identifier }), true, this.ERP_API))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_propostasitensenderecos_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_propostasitensenderecos_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.propostaitemendereco) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_propostasitensenderecos_put', { propostaitem : entity.propostaitem.propostaitem,  'id': entity.propostaitemendereco }, true, this.ERP_API);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_propostasitensenderecos_create', angular.extend(this.constructors), true, this.ERP_API);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.propostaitemendereco) {
                    entity.propostaitemendereco = response.data.propostaitemendereco;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_propostasitensenderecos_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_propostasitensenderecos_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(propostaitem: any, identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_propostasitensenderecos_get', { 'propostaitem': propostaitem,  'id': identifier }, true, this.ERP_API))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_propostasitensenderecos_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
