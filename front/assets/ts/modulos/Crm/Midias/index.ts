...
export class CrmMidiasListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'midia') {
        ...
        this.fields = $rootScope.FIELDS_CrmMidias;
                $scope.$on('crm_midias_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_midias_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
