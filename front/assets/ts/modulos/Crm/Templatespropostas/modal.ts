import angular = require('angular');

export class CrmTemplatespropostasFormModalController {
    static $inject = ['toaster', '$uibModalInstance', 'entity', 'constructors'];

    public action: string;
    public form: any;
    public submitted: boolean = false;
    constructor(public toaster: any, public $uibModalInstance: any, public entity: any, public constructors: any) {
        this.action = entity.templateproposta ? 'update' : 'insert';
    }
    submit() {
        this.submitted = true;
        if (this.form.$valid) {
            this.$uibModalInstance.close(this.entity);
        } else {
            this.toaster.pop({
                type: 'error',
                title: 'Alguns campos do formulário apresentam erros.'
            });
        }
    }
    close() {
        this.$uibModalInstance.dismiss('fechar');
    }
}

export class CrmTemplatespropostasFormService {
    static $inject = ['CrmTemplatespropostas', '$uibModal'];

    constructor(public entityService: any, public $uibModal: any) {
    }

    open(parameters: any, subentity: any) {
        return this.$uibModal.open({
            // template: require('./modal.html'),
            template: require('../../Crm/Templatespropostas/modal.html'),
            controller: 'CrmTemplatespropostasFormModalController',
            controllerAs: 'crm_tmpltsprpsts_frm_dflt_cntrllr',
            windowClass: '',
            size:'lg',
            resolve: {
                entity: () => {
                    if (parameters.templateproposta) {
                        let entity = this.entityService.get(parameters.templatepropostagrupo, parameters.templateproposta);
                        return entity;
                    } else {
                        return angular.copy(subentity);
                    }
                },
                constructors: () => {
                    return parameters;
                }
            }
        });
    }

}
