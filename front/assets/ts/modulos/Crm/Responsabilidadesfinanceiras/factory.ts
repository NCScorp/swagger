...
export class CrmResponsabilidadesfinanceiras {
    ...
    reload() {
        ...
        delete this.filters['responsabilidadefinanceira'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_responsabilidadesfinanceiras_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.responsabilidadefinanceira = this.entities[this.entities.length - 1].responsabilidadefinanceira;
                        this.after.responsabilidadefinanceira = this.entities[this.entities.length - 1].responsabilidadefinanceira;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_responsabilidadesfinanceiras_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].responsabilidadefinanceira === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.responsabilidadefinanceira;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_responsabilidadesfinanceiras_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_responsabilidadesfinanceiras_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_responsabilidadesfinanceiras_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.responsabilidadefinanceira) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_responsabilidadesfinanceiras_put', { negocio : entity.negocio,  'id': entity.responsabilidadefinanceira }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_responsabilidadesfinanceiras_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.responsabilidadefinanceira) {
                    entity.responsabilidadefinanceira = response.data.responsabilidadefinanceira;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_responsabilidadesfinanceiras_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_responsabilidadesfinanceiras_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(negocio: any, identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_responsabilidadesfinanceiras_get', { 'negocio': negocio,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_responsabilidadesfinanceiras_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
