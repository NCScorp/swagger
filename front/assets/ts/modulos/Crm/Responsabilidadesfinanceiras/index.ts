...
export class CrmResponsabilidadesfinanceirasListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'responsabilidadefinanceira') {
        ...
        this.fields = $rootScope.FIELDS_CrmResponsabilidadesfinanceiras;
                $scope.$on('crm_responsabilidadesfinanceiras_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_responsabilidadesfinanceiras_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
