...
export class CrmVinculos {
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_vinculos_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true, true),
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    if (data.length > 0) {
                        ...
                        this.after.vinculo = this.entities[this.entities.length - 1].vinculo;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_vinculos_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].vinculo === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.vinculo;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_vinculos_delete', angular.extend(this.constructors, {'id': $identifier }), true, true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_vinculos_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_vinculos_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        ...
        if (entity.vinculo) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_vinculos_put', {  'id': entity.vinculo }, true, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_vinculos_create', angular.extend(this.constructors), true, true);
        }
        if (!autosave) {
            autosave = false;
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.vinculo) {
                    entity.vinculo = response.data.vinculo;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_vinculos_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_vinculos_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_vinculos_get', {  'id': identifier }, true, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_vinculos_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
