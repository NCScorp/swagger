...
export class CrmVinculosListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'vinculo') {
        ...
        this.fields = $rootScope.FIELDS_CrmVinculos;
                $scope.$on('crm_vinculos_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_vinculos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
