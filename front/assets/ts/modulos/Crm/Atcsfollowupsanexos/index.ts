...
export class CrmAtcsfollowupsanexosListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'negociofollowupanexo') {
        ...
        this.fields = $rootScope.FIELDS_CrmAtcsfollowupsanexos;
                $scope.$on('crm_atcsfollowupsanexos_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_atcsfollowupsanexos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
