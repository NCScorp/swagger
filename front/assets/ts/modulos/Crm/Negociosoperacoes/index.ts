...
export class CrmNegociosoperacoesListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'proposta_operacao') {
        ...
        this.fields = $rootScope.FIELDS_CrmNegociosoperacoes;
                $scope.$on('crm_negociosoperacoes_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_negociosoperacoes_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
