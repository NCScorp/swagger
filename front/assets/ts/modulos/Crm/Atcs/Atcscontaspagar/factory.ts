...
import { ICrmAtcContaPagar } from './classes/icrmatccontapagar';
import { CrmAtcContaPagar } from './classes/crmatccontapagar';

export class CrmAtcContaPagarService {
  ...
  public entity: any = {};
  public loadParams: any = { to_load: 3, busy: false, finished: false };
  ...
  reload() {
    ...
    delete this.filters['atccontaapagar'];
    return this.load();
  }
    ...
    return this.$q((resolve: any, reject: any) => {
      this.$http({
        ...
        url: this.nsjRouting.generate(
          'crm_atcs_contasapagar_index',
          angular.extend({}, constructors, { offset: offset, filter: search }, filters ? filters : this.filters),
          true,
          true
        ),
      ...
      this._load(this.constructors, this.after, this.filter)
        .then((data: any) => {
          if (data.length > 0) {
            ...
            this.after.atccontaapagar = this.entities[this.entities.length - 1].atccontaapagar;
            this.after.atccontaapagar = this.entities[this.entities.length - 1].atccontaapagar;
          }
          if (data.length < 20) {
            this.loadParams.finished = true;
            this.$rootScope.$broadcast('crm_atcscontasapagar_list_finished', this.entities);
          } else {
            this.loadParams.finished = false;
          }
          this.loaded = true;
  ...
  find(identifier: string) {
    for (let i in this.entities) {
      if (this.entities[i].atccontaapagar === identifier) {
  ...
  delete($identifier: any, force: boolean) {
    if (typeof $identifier === 'object') {
      $identifier = $identifier.atccontaapagar;
    }
    if ($identifier) {
        ...
        this.$http
          .delete(
            this.nsjRouting.generate(
              'crm_atcscontasapagar_delete',
              angular.extend(this.constructors, { id: $identifier }),
              true
            )
          )
          .then((response: any) => {
            this.$rootScope.$broadcast('crm_atcscontasapagar_deleted', {
              entity: this.entity,
              response: response,
            });
          })
          .catch((response: any) => {
            this.$rootScope.$broadcast('crm_atcscontasapagar_delete_error', {
              entity: this.entity,
              response: response,
            });
          });
  ...
  _save(entity: any, autosave: boolean) {
    let method, url;
    if (entity.atccontaapagar) {
      method = 'PUT';
      url = this.nsjRouting.generate(
        'crm_atcs_contasapagar_put',
        { atc: entity.atc, id: entity.atccontaapagar },
        true,
        true
      );
    } else {
      method = 'POST';
      url = this.nsjRouting.generate('crm_atcscontasapagar_create', angular.extend(this.constructors), true);
    }
    if (!autosave) {
      autosave = false;
  ...
  save(entity: any, autosave: boolean) {
    this._save(entity, autosave)
      .then((response: any) => {
        if (response.data.atccontaapagar) {
          entity.atccontaapagar = response.data.atccontaapagar;
        }
        entity.$$_saved = true;
        this.$rootScope.$broadcast('crm_atcscontasapagar_submitted', {
          entity: entity,
          response: response,
          autosave: autosave,
        });
      ...
      .catch((response: any) => {
        this.$rootScope.$broadcast('crm_atcscontasapagar_submit_error', {
          entity: entity,
          response: response,
          autosave: autosave,
        });
  ...
  get(negocio: any, identifier: string) {
    return this.$q((resolve: any, reject: any) => {
      this.$http
        .get(this.nsjRouting.generate('crm_atcs_contasapagar_get', { negocio: negocio, id: identifier }, true, true))
        .then((response: any) => {
          this.$rootScope.$broadcast('crm_atcscontasapagar_loaded', response.data);
          resolve(response.data);
        })
        .catch((response: any) => {
          reject(response);
  ...
  /**
   * Busca lista de todos as contas a pagar do atendimento
   * @param atc
   */
  public getAll(atc: string, filtros: any = {}): Promise<CrmAtcContaPagar[]> {
    ...
    return new Promise((resolve, reject) => {
      this.$http({
        ...
        url: this.nsjRouting.generate(
          'crm_atcs_contasapagar_index',
          angular.extend(filtros, this.constructors),
          true,
          true
        ),
      })
        .then((response: any) => {
          const arrDados = CrmAtcContaPagar.getFromListaApi(response.data);
          resolve(arrDados);
          this.$rootScope.$broadcast('crm_atcscontasapagar_loaded', arrDados);
        })
        .catch((erro: any) => {
          reject(erro);
        });
   ...
   * @param entity
   */
  public put(entity: ICrmAtcContaPagar): Promise<void> {
    return new Promise((resolve, reject) => {
  ...
  gerarContaPagar(entity: ICrmAtcContaPagar) {
    return this.$q((resolve: any, reject: any) => {
      this.$http
        .post(
          this.nsjRouting.generate(
            'crm_atcs_contasapagar_gerar_conta_a_pagar',
            { atc: entity.atc, id: entity.atccontaapagar },
            true,
            true
          ),
  ...
  desfazerContaPagar(entity: ICrmAtcContaPagar) {
    return this.$q((resolve: any, reject: any) => {
      this.$http
        .post(
          this.nsjRouting.generate(
            'crm_atcs_contasapagar_desfazer_conta_a_pagar',
            { atc: entity.atc, id: entity.atccontaapagar },
            true,
            true
          ),
