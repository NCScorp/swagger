...
export class CrmAtcsdadosseguradorasListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'negociodadosseguradora') {
        ...
        this.fields = $rootScope.FIELDS_CrmAtcsdadosseguradoras;
                $scope.$on('crm_atcsdadosseguradoras_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_atcsdadosseguradoras_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
