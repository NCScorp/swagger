...
export class CrmAtcsfollowups {
    ...
    reload() {
        ...
        delete this.filters['negociofollowup'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_atcs_followups_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true, true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.negociofollowup = this.entities[this.entities.length - 1].negociofollowup;
                        this.after.negociofollowup = this.entities[this.entities.length - 1].negociofollowup;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_atcsfollowups_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].negociofollowup === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.negociofollowup;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_atcs_followups_delete', angular.extend(this.constructors, {'id': $identifier }), true, true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_atcsfollowups_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_atcsfollowups_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.negociofollowup) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_atcs_followups_put', { negocio : entity.negocio.negocio,  'id': entity.negociofollowup }, true, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_atcs_followups_create', angular.extend(this.constructors), true, true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.negociofollowup) {
                    entity.negociofollowup = response.data.negociofollowup;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_atcsfollowups_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_atcsfollowups_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(negocio: any, identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_atcs_followups_get', { 'negocio': negocio,  'id': identifier }, true, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_atcsfollowups_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
