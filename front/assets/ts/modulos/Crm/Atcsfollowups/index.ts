...
export class CrmAtcsfollowupsListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'CrmAtcsfollowups', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'negociofollowup') {
        ...
        this.fields = $rootScope.FIELDS_CrmAtcsfollowups;
                $scope.$on('crm_atcsfollowups_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_atcsfollowups_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
