...
export class CrmListadavezvendedoresitensListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['posicao', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'listadavezvendedoritem') {
        ...
        this.fields = $rootScope.FIELDS_CrmListadavezvendedoresitens;
                $scope.$on('crm_listadavezvendedoresitens_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_listadavezvendedoresitens_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
