...
export class CrmTemplatespropostascapituloscomposicoes {
    ...
    reload() {
        ...
        delete this.filters['templatepropostacomposicao'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_templatespropostascapituloscomposicoes_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.templatepropostacomposicao = this.entities[this.entities.length - 1].templatepropostacomposicao;
                        this.after.templatepropostacomposicao = this.entities[this.entities.length - 1].templatepropostacomposicao;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_templatespropostascapituloscomposicoes_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].templatepropostacomposicao === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.templatepropostacomposicao;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_templatespropostascapituloscomposicoes_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_templatespropostascapituloscomposicoes_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_templatespropostascapituloscomposicoes_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.templatepropostacomposicao) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_templatespropostascapituloscomposicoes_put', { templatepropostacapitulo : entity.templatepropostacapitulo,  'id': entity.templatepropostacomposicao }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_templatespropostascapituloscomposicoes_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.templatepropostacomposicao) {
                    entity.templatepropostacomposicao = response.data.templatepropostacomposicao;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_templatespropostascapituloscomposicoes_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_templatespropostascapituloscomposicoes_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(templatepropostacapitulo: any, identifier : string) {
                
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_templatespropostascapituloscomposicoes_get', { 'templatepropostacapitulo': templatepropostacapitulo,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_templatespropostascapituloscomposicoes_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
