...
export class CrmPropostasitensfuncoesFormController {
   ...
   this.$scope.$on('crm_propostasitensfuncoes_deleted', (event: any, args: any) => {
    ...
    $scope.$watch('crm_prpststnsfncs_frm_cntrllr.entity', (newValue: any, oldValue: any) => {
        ...
        for (var i in $stateParams) {
            if (i !== 'entity' && typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'propostaitemfuncao') {
                ...
                this.constructors = entityService.constructors;
                                                $scope.$on('crm_propostasitensfuncoes_submitted', (event: any, args: any) => {
                            let insertSuccess = 'Sucesso ao inserir Crm\Propostasitensfuncoes!';
                let updateSuccess = 'Sucesso ao atualizar Crm\Propostasitensfuncoes!';

                this.toaster.pop({
                    type: 'success',
                    title: args.response.config.method === 'PUT' ? updateSuccess : insertSuccess
                });
                this.$state.go('crm_propostasitensfuncoes_show', angular.extend({}, this.entityService.constructors, {'propostaitemfuncao': args.entity.propostaitemfuncao}));
                    });
        $scope.$on('crm_propostasitensfuncoes_submit_error', (event: any, args: any) => {
       ...
       if (typeof (args.response.data.message) !== 'undefined' && args.response.data.message) {
            ...
            {
                type: 'error',
                title: args.response.config.method === 'PUT' ? 'Ocorreu um erro ao atualizar Crm\Propostasitensfuncoes!' : 'Ocorreu um erro ao inserir Crm\Propostasitensfuncoes!'            });
        }
    }
