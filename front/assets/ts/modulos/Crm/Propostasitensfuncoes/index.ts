...
export class CrmPropostasitensfuncoesListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'propostaitemfuncao') {
        ...
        this.fields = $rootScope.FIELDS_CrmPropostasitensfuncoes;
                $scope.$on('crm_propostasitensfuncoes_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_propostasitensfuncoes_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
