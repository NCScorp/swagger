...
import { ModalExclusaoPropItemFuncaoService } from './modal-exclusao';

export class CrmPropostasitensfuncoes {
    static $inject = ['$http', 'nsjRouting', '$rootScope', '$q',
        'modalExclusaoPropItemFuncaoService', 'toaster'];
    public entities: any[] = [];
    ...
    constructor(public $http: angular.IHttpService, public nsjRouting: any, public $rootScope: angular.IScope, public $q: angular.IQService,
        public ModalExclusaoPropItemFuncaoService: ModalExclusaoPropItemFuncaoService, public toaster: any) {
    }
    ...
    reload() {
        ...
        delete this.filters['propostaitemfuncao'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_propostasitens_funcoes_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true, true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.propostaitemfuncao = this.entities[this.entities.length - 1].propostaitemfuncao;
                        this.after.propostaitemfuncao = this.entities[this.entities.length - 1].propostaitemfuncao;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_propostasitensfuncoes_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].propostaitemfuncao === identifier) {
    ...
    // Abre modal de confirmação de exclusão da proposta item funcao (profissional)
    abrirModalExclusao(propostaitemfuncao: any){

        let modal = this.ModalExclusaoPropItemFuncaoService.open(propostaitemfuncao);
        modal.result.then((response: any) => {
            this.delete(propostaitemfuncao, true); 
        })
        .catch((error: any) => {
    ...
    delete($parameters: any, force: boolean) {
        if ($parameters.id) {
            if (force) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_propostasitens_funcoes_delete', angular.extend(this.constructors, {'id': $parameters.id, 'negocio': $parameters.negocio, 'proposta': $parameters.proposta }), true, true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_propostasitensfuncoes_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_propostasitensfuncoes_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.propostaitemfuncao) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_propostasitens_funcoes_put', { propostaitem : entity.propostaitem,  'id': entity.propostaitemfuncao, 'negocio': entity.negocio, 'proposta': entity.proposta }, true, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_propostasitens_funcoes_create', angular.extend(this.constructors, {'negocio': entity.negocio, 'proposta': entity.proposta}), true, true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.propostaitemfuncao) {
                    entity.propostaitemfuncao = response.data.propostaitemfuncao;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_propostasitensfuncoes_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_propostasitensfuncoes_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(propostaitem: any, identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_propostasitens_funcoes_get', { 'propostaitem': propostaitem,  'id': identifier }, true, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_propostasitensfuncoes_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
