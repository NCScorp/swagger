...
export class CrmOrcamentosListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['propostaitem', 'propostaitemfamilia', 'propostaitemfuncao', 'execucaodeservico', 'itemfaturamento', 'fornecedor', 'propostaitem.negocio', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'orcamento') {
        ...
        this.fields = $rootScope.FIELDS_CrmOrcamentos;
                $scope.$on('crm_orcamentos_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_orcamentos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
