...
import { IOrcamento } from './classes/iorcamento';

export class CrmOrcamentos {
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_orcamentos_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true, true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.orcamento = this.entities[this.entities.length - 1].orcamento;
                        this.after.orcamento = this.entities[this.entities.length - 1].orcamento;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_orcamentos_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].orcamento === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.orcamento;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_orcamentos_delete', angular.extend(this.constructors, {'id': $identifier }), true, true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_orcamentos_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_orcamentos_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        ...
        if (entity.orcamento) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_orcamentos_put', {'atc': entity.atc, 'id': entity.orcamento }, true, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_orcamentos_create', angular.extend(this.constructors, {'atc': entity.atc}), true, true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.orcamento) {
                    entity.orcamento = response.data.orcamento;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_orcamentos_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_orcamentos_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('crm_orcamentos_enviar', {'id': entity.orcamento }, true, true),
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('crm_orcamentos_aprovar', {'id': entity.orcamento }, true, true),
            data: angular.copy(entity)
            })
            .then((response: any) => {
                resolve(response.data);
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('crm_orcamentos_renegociar', {'id': entity.orcamento }, true, true),
            data: angular.copy(entity)
            })
            .then((response: any) => {
                resolve(response.data);
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('crm_orcamentos_reprovar', {'id': entity.orcamento }, true, true),
            data: angular.copy(entity)
            })
            .then((response: any) => {
                resolve(response.data);
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_orcamentos_get', {  'id': identifier }, true, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_orcamentos_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
     ...
     * @param entity 
     */
    public put(entity: IOrcamento): Promise<IOrcamento> {
     ...
     * @param entity 
     */
    public create(entity: IOrcamento): Promise<IOrcamento> {
        return new Promise((resolve, reject) => {
            this._save(entity, false).then((response: any) => {
                resolve(response.data);
            }).catch((response: any) => {
     ...
     * @param orcamentoObj 
     */
    excluir(orcamentoObj: any): Promise<any>{
        return new Promise((resolve, reject) => {
            const url = this.nsjRouting.generate('crm_orcamentos_delete', angular.extend(this.constructors, {'atc': orcamentoObj.dados.atc, id: orcamentoObj.dados.orcamento}), true);
            this.$http.delete(url).then((response: any) => {
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response.data);
