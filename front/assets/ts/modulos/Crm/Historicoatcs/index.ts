...
export class CrmHistoricoatcsListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'CrmHistoricoatcs', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['secao', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'historiconegocio') {
        ...
        this.fields = $rootScope.FIELDS_CrmHistoricoatcs;
                $scope.$on('crm_historicoatcs_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_historicoatcs_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
