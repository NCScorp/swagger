...
export class CrmHistoricoatcs {
    ...
    reload() {
        ...
        delete this.filters['historiconegocio'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_atcs_historicosindex', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true, true),
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    if (data.length > 0) {
                        ...
                        this.after.historiconegocio = this.entities[this.entities.length - 1].historiconegocio;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_historicoatcs_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].historiconegocio === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.historiconegocio;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_historicoatcs_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_historicoatcs_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_historicoatcs_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.historiconegocio) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_historicoatcs_put', { negocio : entity.negocio,  'id': entity.historiconegocio }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_historicoatcs_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.historiconegocio) {
                    entity.historiconegocio = response.data.historiconegocio;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_historicoatcs_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_historicoatcs_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(negocio: any, identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_atcs_historicosget', { 'negocio': negocio,  'id': identifier }, true, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_historicoatcs_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
