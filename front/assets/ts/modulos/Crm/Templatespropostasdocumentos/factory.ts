...
export class CrmTemplatespropostasdocumentos {
    ...
    reload() {
        ...
        delete this.filters['templatepropostadocumento'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_templatespropostasdocumentos_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.templatepropostadocumento = this.entities[this.entities.length - 1].templatepropostadocumento;
                        this.after.templatepropostadocumento = this.entities[this.entities.length - 1].templatepropostadocumento;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_templatespropostasdocumentos_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].templatepropostadocumento === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.templatepropostadocumento;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_templatespropostasdocumentos_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_templatespropostasdocumentos_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_templatespropostasdocumentos_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.templatepropostadocumento) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_templatespropostasdocumentos_put', { templateproposta : entity.templateproposta,  'id': entity.templatepropostadocumento }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_templatespropostasdocumentos_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.templatepropostadocumento) {
                    entity.templatepropostadocumento = response.data.templatepropostadocumento;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_templatespropostasdocumentos_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_templatespropostasdocumentos_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(templateproposta: any, identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_templatespropostasdocumentos_get', { 'templateproposta': templateproposta,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_templatespropostasdocumentos_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
