...
export class CrmListadavezvendedoresListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'listadavezvendedor') {
        ...
        this.fields = $rootScope.FIELDS_CrmListadavezvendedores;
                $scope.$on('crm_listadavezvendedores_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_listadavezvendedores_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
