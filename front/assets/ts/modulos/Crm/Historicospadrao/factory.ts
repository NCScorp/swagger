...
export class CrmHistoricospadrao {
    ...
    reload() {
        ...
        delete this.filters['historicopadrao'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_historicospadrao_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true, true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.historicopadrao = this.entities[this.entities.length - 1].historicopadrao;
                        this.after.historicopadrao = this.entities[this.entities.length - 1].historicopadrao;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_historicospadrao_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].historicopadrao === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.historicopadrao;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_historicospadrao_delete', angular.extend(this.constructors, {'id': $identifier }), true, true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_historicospadrao_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_historicospadrao_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.historicopadrao) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_historicospadrao_put', {  'id': entity.historicopadrao }, true, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_historicospadrao_create', angular.extend(this.constructors), true, true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.historicopadrao) {
                    entity.historicopadrao = response.data.historicopadrao;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_historicospadrao_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_historicospadrao_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_historicospadrao_get', {  'id': identifier }, true, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_historicospadrao_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
