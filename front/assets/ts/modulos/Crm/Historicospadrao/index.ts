...
export class CrmHistoricospadraoListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['tipo', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'historicopadrao') {
        ...
        this.fields = $rootScope.FIELDS_CrmHistoricospadrao;
                $scope.$on('crm_historicospadrao_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_historicospadrao_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
