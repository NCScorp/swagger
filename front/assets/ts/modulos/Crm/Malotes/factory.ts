...
export class CrmMalotes {
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_malotes_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.status = this.entities[this.entities.length - 1].status;
                        this.after.malote = this.entities[this.entities.length - 1].malote;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_malotes_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].malote === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.malote;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_malotes_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_malotes_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_malotes_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        ...
        if (entity.malote) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_malotes_put', {  'id': entity.malote }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_malotes_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.malote) {
                    entity.malote = response.data.malote;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_malotes_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_malotes_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('crm_malotes_malote_aprova', {  'id': entity.malote }, true),
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('crm_malotes_malote_enviar', {  'id': entity.malote }, true),
            data: angular.copy(entity)
            })
            .then((response: any) => {
                resolve(response.data);
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('crm_malotes_malotecancelaenvio', {  'id': entity.malote }, true),
            data: angular.copy(entity)
            })
            .then((response: any) => {
                resolve(response.data);
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('crm_malotes_malote_editar_envio', {  'id': entity.malote }, true),
            data: angular.copy(entity)
            })
            .then((response: any) => {
                resolve(response.data);
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_malotes_get', {  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_malotes_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
