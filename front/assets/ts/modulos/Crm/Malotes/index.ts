...
export class CrmMalotesListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'malote') {
        ...
        this.fields = $rootScope.FIELDS_CrmMalotes;
                $scope.$on('crm_malotes_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_malotes_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
