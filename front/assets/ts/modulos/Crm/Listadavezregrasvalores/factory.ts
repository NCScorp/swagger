...
import { IListadavezregrasvaloresApi } from './classes';

export class CrmListadavezregrasvalores {
    ...
    reload() {
        ...
        delete this.filters['listadavezregravalor'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_listadavezregrasvalores_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true, true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.listadavezregravalor = this.entities[this.entities.length - 1].listadavezregravalor;
                        this.after.listadavezregravalor = this.entities[this.entities.length - 1].listadavezregravalor;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_listadavezregrasvalores_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].listadavezregravalor === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.listadavezregravalor;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_listadavezregrasvalores_delete', angular.extend(this.constructors, {'id': $identifier }), true, true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_listadavezregrasvalores_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_listadavezregrasvalores_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.listadavezregravalor) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_listadavezregrasvalores_put', { listadavezregra : entity.listadavezregra,  'id': entity.listadavezregravalor }, true, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_listadavezregrasvalores_create', angular.extend(this.constructors), true, true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.listadavezregravalor) {
                    entity.listadavezregravalor = response.data.listadavezregravalor;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_listadavezregrasvalores_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_listadavezregrasvalores_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(listadavezregra: any, identifier : string) {
                
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_listadavezregrasvalores_get', { 'listadavezregra': listadavezregra,  'id': identifier }, true, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_listadavezregrasvalores_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
    ...
    /**
     * Retorna a lista de valores das regras da lista da vez
     */
    getAll(listadavezregra: string): Promise<IListadavezregrasvaloresApi[]> {
        return new Promise((resolve, reject) => {
            // Monto a url
            const url = this.nsjRouting.generate('crm_listadavezregrasvalores_index', angular.extend(this.constructors, {listadavezregra}), true, true);
