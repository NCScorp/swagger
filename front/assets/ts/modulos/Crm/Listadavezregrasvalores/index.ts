...
export class CrmListadavezregrasvaloresListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'listadavezregravalor') {
        ...
        this.fields = $rootScope.FIELDS_CrmListadavezregrasvalores;
                $scope.$on('crm_listadavezregrasvalores_deleted', (event: any) => {
        ...
        $rootScope.$on('crm_listadavezregrasvalores_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
