...
export class CrmNegocios {
            ...
            for ( let fil in filter.filters ) {
                if (typeof(filter.filters[fil]) !== 'object') {
                    ...
                    filters[fil] = filter.filters[fil];
                        ...
                        if ( filter.filterLookup && filter.filterLookup.clientecaptador ) {
                filters.clientecaptador = filter.filterLookup.clientecaptador.vendedor_id;
            }
                        if ( filter.filterLookup && filter.filterLookup.vendedor ) {
                filters.vendedor = filter.filterLookup.vendedor.vendedor_id;
            }
                    } else if (filter == null) {
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('crm_negocios_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.documento = this.entities[this.entities.length - 1].documento;
                        this.after.documento = this.entities[this.entities.length - 1].documento;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('crm_negocios_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].documento === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.documento;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('crm_negocios_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('crm_negocios_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('crm_negocios_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        ...
        if (entity.documento) {
            method = 'PUT';
            url = this.nsjRouting.generate('crm_negocios_put', {  'id': entity.documento }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('crm_negocios_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.documento) {
                    entity.documento = response.data.documento;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('crm_negocios_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('crm_negocios_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('crm_negocios_pre_negocio_qualificar', {  'id': entity.documento }, true),
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('crm_negocios_pre_negocio_desqualificar', {  'id': entity.documento }, true),
            data: angular.copy(entity)
            })
            .then((response: any) => {
                resolve(response.data);
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('crm_negocios_get', {  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('crm_negocios_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
    ...
    /**
     * Retorna a lista da vez referente a um negócio
     * @param id 
     */
    getListaDaVez(id): Promise<IListadavezvendedoritem>{
        return new Promise((resolve, reject) => {
            // Monto parâmetros
            const params = {...this.constructors, id}
            // Monto a url
            const url = this.nsjRouting.generate('crm_negocios_buscar_vendedor_da_lista_da_vez', params, true);
