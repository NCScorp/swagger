...
export class TiposServicosService {
    ...
    reload() {
        ...
        delete this.filters['tiposervico'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('servicos_tiposservicos_index', angular.extend({}, constructors, { 'offset': offset, 'filter': search }, filters ? filters : this.filters), true, this.rotaErpApi),
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    if (data.length > 0) {
                        ...
                        this.after.tiposervico = this.entities[this.entities.length - 1].tiposervico;
                        this.after.tiposervico = this.entities[this.entities.length - 1].tiposervico;
                    }
                    if (data.length < 20) {
                        this.loadParams.finished = true;
                        this.$rootScope.$broadcast('tiposservicos_list_finished', this.entities);
                    } else {
                        this.loadParams.finished = false;
                    }
                    this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].tiposervico === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof ($identifier) === 'object') {
            $identifier = $identifier.tiposervico;
        }
        if ($identifier) {
            if (force || confirm('Tem certeza que deseja deletar?')) {
                this.loading_deferred = this.$q.defer();
                this.$http
                    .delete(this.nsjRouting.generate('servicos_tiposservicos_delete', angular.extend(this.constructors, { 'id': $identifier }), true, this.rotaErpApi),
                        { timeout: this.loading_deferred.promise })
                    .then((response: any) => {
                        this.$rootScope.$broadcast('tiposservicos_deleted', {
                            entity: this.entity,
                            response: response
                        });
                    })
                    .catch((response: any) => {
                        this.$rootScope.$broadcast('tiposservicos_delete_error', {
                            entity: this.entity,
                            response: response
                        });
                    });
    ...
    _save(entity: any, autosave: boolean = false) {
        let method, url;
        if (entity.tiposervico) {
            method = 'PUT';
            url = this.nsjRouting.generate('servicos_tiposservicos_put', { 'id': entity.tiposervico }, true, this.rotaErpApi);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('servicos_tiposservicos_create', angular.extend(this.constructors), true, this.rotaErpApi);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean = false) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.tiposervico) {
                    entity.tiposervico = response.data.tiposervico;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('tiposservicos_submitted', {
                    entity: entity,
                    response: response,
                    autosave: autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('tiposservicos_submit_error', {
                    entity: entity,
                    response: response,
                    autosave: autosave
                });
    ...
    get(identifier: string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
                .get(this.nsjRouting.generate('servicos_tiposservicos_get', { 'id': identifier }, true, this.rotaErpApi),
                    { timeout: this.loading_deferred.promise })
                .then((response: any) => {
                    this.$rootScope.$broadcast('tiposservicos_loaded', response.data);
                    resolve(response.data);
                })
                .catch((response: any) => {
                    reject(response);
