...
import { NsjStorage } from '@nsj/core/utils/storage';

export class OperacoesOrdensServicosService {
    ...
    reload() {
        ...
        delete this.filters['operacaoordemservico'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('servicos_ordensservicos_operacoes_index', angular.extend({}, constructors, {
                    'offset': offset,
                    'filter': search
                }, filters ? filters : this.filters), true, this.rotaErpApi),
    ...
    load() {
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    if (data.length > 0) {
                        ...
                        this.after.operacaoordemservico = this.entities[this.entities.length - 1].operacaoordemservico;
                        this.after.operacaoordemservico = this.entities[this.entities.length - 1].operacaoordemservico;
                    }
                    if (data.length < 20) {
                        this.loadParams.finished = true;
                        this.$rootScope.$broadcast('operacoesordensservicos_list_finished', this.entities);
                    } else {
                        this.loadParams.finished = false;
                    }
                    this.loaded = true;
        ...
        NsjStorage.insert('operacoesordensservicos', this.entities);
    ...
    find(identifier: string) {
        const entities = NsjStorage.get<any>('operacoesordensservicos') || [];
        return entities.find(entity => entity.operacaoordemservico === identifier);
    }

    delete($identifier: any, force: boolean) {
        if (typeof ($identifier) === 'object') {
            $identifier = $identifier.operacaoordemservico;
        }
        if ($identifier) {
            if (force || confirm('Tem certeza que deseja deletar?')) {
                this.loading_deferred = this.$q.defer();
                this.$http
                    .delete(this.nsjRouting.generate('servicos_ordensservicos_operacoes_delete', angular.extend(this.constructors, {'id': $identifier}), true, this.rotaErpApi),
                        {timeout: this.loading_deferred.promise})
                    .then((response: any) => {
                        this.$rootScope.$broadcast('operacoesordensservicos_deleted', {
                            entity: this.entity,
                            response: response
                        });
                    })
                    .catch((response: any) => {
                        this.$rootScope.$broadcast('operacoesordensservicos_delete_error', {
                            entity: this.entity,
                            response: response
                        });
                    });
    ...
    _save(entity: any, autosave: boolean = false) {
        let method, url;
        if (entity.operacaoordemservico) {
            method = 'PUT';
            url = this.nsjRouting.generate('servicos_ordensservicos_operacoes_put', {'id': entity.operacaoordemservico}, true, this.rotaErpApi);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('servicos_ordensservicos_operacoes_create', angular.extend(this.constructors), true, this.rotaErpApi);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean = false) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.operacaoordemservico) {
                    entity.operacaoordemservico = response.data.operacaoordemservico;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('operacoesordensservicos_submitted', {
                    entity: entity,
                    response: response,
                    autosave: autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('operacoesordensservicos_submit_error', {
                    entity: entity,
                    response: response,
                    autosave: autosave
                });
    ...
    get(identifier: string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
                .get(this.nsjRouting.generate('servicos_ordensservicos_operacoes_get', {'id': identifier}, true, this.rotaErpApi),
                    {timeout: this.loading_deferred.promise})
                .then((response: any) => {
                    this.$rootScope.$broadcast('operacoesordensservicos_loaded', response.data);
                    resolve(response.data);
                })
                .catch((response: any) => {
                    reject(response);
    ...
    loadAll() {
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('servicos_ordensservicos_operacoes_index', angular.extend({}, {}, {
                    'offset': '',
                    'filter': ''
                }, {}), true, this.rotaErpApi)
            })
                .then((response: any) => {
                    NsjStorage.insert('operacoesordensservicos',angular.copy(response.data));
