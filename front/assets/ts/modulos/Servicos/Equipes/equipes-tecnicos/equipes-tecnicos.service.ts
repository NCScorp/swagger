...
export class EquipesTecnicosService {
    ...
    reload() {
        ...
        delete this.filters['equipetecnicotecnico'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('servicos_equipes_tecnicos_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true, this.rotaErpApi),
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    if (data.length > 0) {
                        ...
                        this.$rootScope.$broadcast('equipes_equipestecnicos_list_finished', this.entities);
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].equipetecnicotecnico === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.equipetecnicotecnico;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                .delete(this.nsjRouting.generate('servicos_equipes_tecnicos_delete', angular.extend(this.constructors, {'id': $identifier }), true, this.rotaErpApi),
                        { timeout: this.loading_deferred.promise })
                .then((response: any) => {
                    this.$rootScope.$broadcast('equipes_equipestecnicos_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('equipes_equipestecnicos_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean = false) {
        let method, url;
        if (entity.equipetecnicotecnico) {
            method = 'PUT';
            url = this.nsjRouting.generate('servicos_equipes_tecnicos_put', { equipetecnico : entity.equipetecnico.equipetecnico,  'id': entity.equipetecnicotecnico }, true, this.rotaErpApi);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('servicos_equipes_tecnicos_create', angular.extend(this.constructors), true, this.rotaErpApi);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean =  false) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.equipetecnicotecnico) {
                    entity.equipetecnicotecnico = response.data.equipetecnicotecnico;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('equipes_equipestecnicos_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('equipes_equipestecnicos_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(equipetecnico: any, identifier : string) {
        this.loading_deferred = this.$q.defer();

        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('servicos_equipes_tecnicos_get', { 'equipetecnico': equipetecnico,  'id': identifier }, true, this.rotaErpApi),
            { timeout : this.loading_deferred.promise })
            .then((response: any) => {
                this.$rootScope.$broadcast('equipes_equipestecnicos_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
