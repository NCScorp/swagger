...
export class TecnicosLogoService {
    ...
    reload() {
        ...
        delete this.filters['tecnicologo'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('tecnicos_tecnicoslogos_index', angular.extend({}, constructors, { 'offset': offset, 'filter': search }, filters ? filters : this.filters), true, this.rotaErpApi),
        ...
        if (!this.loadParams.busy && !this.loadParams.finished && this.loadParams.to_load > 0 && !this.loaded) {
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    ...
                    this.$rootScope.$broadcast('tecnicos_tecnicoslogos_list_finished', this.entities);
                    this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].tecnicologo === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof ($identifier) === 'object') {
            $identifier = $identifier.tecnicologo;
        }
        if ($identifier) {
            if (force) {
                this.loading_deferred = this.$q.defer();
                this.$http
                    .delete(this.nsjRouting.generate('tecnicos_tecnicoslogos_delete', angular.extend(this.constructors, { 'id': $identifier }), true, this.rotaErpApi))
                    .then((response: any) => {
                        this.$rootScope.$broadcast('tecnicos_tecnicoslogos_deleted', {
                            entity: this.entity,
                            response: response
                        });
                    })
                    .catch((response: any) => {
                        this.$rootScope.$broadcast('tecnicos_tecnicoslogos_delete_error', {
                            entity: this.entity,
                            response: response
                        });
                    });
    ...
    _save(entity: any, autosave: boolean = false) {
        let method, url;
        if (entity.tecnicologo) {
            method = 'PUT';
            url = this.nsjRouting.generate('tecnicos_tecnicoslogos_put', { tecnico: entity.tecnico, 'id': entity.tecnicologo }, true, this.rotaErpApi);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('tecnicos_tecnicoslogos_create', angular.extend(this.constructors), true, this.rotaErpApi);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean = false) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.tecnicologo) {
                    entity.tecnicologo = response.data.tecnicologo;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('tecnicos_tecnicoslogos_submitted', {
                    entity: entity,
                    response: response,
                    autosave: autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('tecnicos_tecnicoslogos_submit_error', {
                    entity: entity,
                    response: response,
                    autosave: autosave
                });
    ...
    get(tecnico: any, identifier: string) {

        return this.$q((resolve: any, reject: any) => {
            this.$http
                .get(this.nsjRouting.generate('tecnicos_tecnicoslogos_get', { 'tecnico': tecnico, 'id': identifier }, true, this.rotaErpApi))
                .then((response: any) => {
                    this.$rootScope.$broadcast('tecnicos_tecnicoslogos_loaded', response.data);
                    resolve(response.data);
                })
                .catch((response: any) => {
                    reject(response);
