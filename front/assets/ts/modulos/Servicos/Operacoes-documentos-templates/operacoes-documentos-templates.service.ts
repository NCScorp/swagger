...
export class OperacoesDocumentosTemplatesService {
    ...
    reload() {
        ...
        delete this.filters['operacaodocumentotemplate'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('operacoesdocumentostemplates_index', angular.extend({}, constructors, {
                    'offset': offset,
                    'filter': search
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    if (data.length > 0) {
                        ...
                        this.after.operacaodocumentotemplate = this.entities[this.entities.length - 1].operacaodocumentotemplate;
                        this.after.operacaodocumentotemplate = this.entities[this.entities.length - 1].operacaodocumentotemplate;
                    }
                    if (data.length < 20) {
                        this.loadParams.finished = true;
                        this.$rootScope.$broadcast('operacoesdocumentostemplates_list_finished', this.entities);
                    } else {
                        this.loadParams.finished = false;
                    }
                    this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].operacaodocumentotemplate === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof ($identifier) === 'object') {
            $identifier = $identifier.operacaodocumentotemplate;
        }
        if ($identifier) {
            if (force || confirm('Tem certeza que deseja deletar?')) {
                this.loading_deferred = this.$q.defer();
                this.$http
                    .delete(this.nsjRouting.generate('operacoesdocumentostemplates_delete', angular.extend(this.constructors, {'id': $identifier}), true, this.rotaErpApi))
                    .then((response: any) => {
                        this.$rootScope.$broadcast('operacoesdocumentostemplates_deleted', {
                            entity: this.entity,
                            response: response
                        });
                    })
                    .catch((response: any) => {
                        this.$rootScope.$broadcast('operacoesdocumentostemplates_delete_error', {
                            entity: this.entity,
                            response: response
                        });
                    });
    ...
    _save(entity: any, autosave: boolean = false) {
        let method, url;
        if (entity.operacaodocumentotemplate) {
            method = 'PUT';
            url = this.nsjRouting.generate('operacoesdocumentostemplates_put', {
                operacaoordemservico: entity.operacaoordemservico.operacaoordemservico,
                'id': entity.operacaodocumentotemplate
            }, true, this.rotaErpApi);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('operacoesdocumentostemplates_create', angular.extend(this.constructors), true, this.rotaErpApi);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean = false) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.operacaodocumentotemplate) {
                    entity.operacaodocumentotemplate = response.data.operacaodocumentotemplate;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('operacoesdocumentostemplates_submitted', {
                    entity: entity,
                    response: response,
                    autosave: autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('operacoesdocumentostemplates_submit_error', {
                    entity: entity,
                    response: response,
                    autosave: autosave
                });
    ...
    get(operacaoordemservico: any, identifier: string) {

        return this.$q((resolve: any, reject: any) => {
            this.$http
                .get(this.nsjRouting.generate('operacoesdocumentostemplates_get', {
                    'operacaoordemservico': operacaoordemservico,
                    'id': identifier
                }, true, this.rotaErpApi))
                .then((response: any) => {
                    this.$rootScope.$broadcast('operacoesdocumentostemplates_loaded', response.data);
                    resolve(response.data);
                })
                .catch((response: any) => {
                    reject(response);
