...
export class ServicosTecnicosChecklistService {
    ...
    reload() {
        ...
        delete this.filters['servicotecnicochecklist'];
        return this.load();
    }
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                ...
                url: this.nsjRouting.generate(
                    'servicostecnicos_servicostecnicoschecklist_index', 
        ...
        if (!this.loadParams.busy && !this.loadParams.finished && this.loadParams.to_load > 0 && !this.loaded) {
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    ...
                    this.$rootScope.$broadcast('servicostecnicos_servicostecnicoschecklist_list_finished', this.entities);
                    this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].servicotecnicochecklist === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof ($identifier) === 'object') {
            $identifier = $identifier.servicotecnicochecklist;
        }
        if ($identifier) {
            if (force || confirm('Tem certeza que deseja deletar?')) {
                this.loading_deferred = this.$q.defer();
                this.$http
                    .delete(this.nsjRouting.generate(
                        'servicostecnicos_servicostecnicoschecklist_delete', 
                        angular.extend(this.constructors, { 'id': $identifier }), 
                        true, this.rotaErpApi),
                        { timeout: this.loading_deferred.promise })
                    .then((response: any) => {
                        this.$rootScope.$broadcast('servicostecnicos_servicostecnicoschecklist_deleted', {
                            entity: this.entity,
                            response: response
                        });
                    })
                    .catch((response: any) => {
                        this.$rootScope.$broadcast('servicostecnicos_servicostecnicoschecklist_delete_error', {
                            entity: this.entity,
                            response: response
                        });
                    });
    ...
    _save(entity: any, autosave: boolean =  false) {
        let method, url;
        if (entity.servicotecnicochecklist) {
            method = 'PUT';
            url = this.nsjRouting.generate(
                'servicostecnicos_servicostecnicoschecklist_put', 
                { servicotecnico: entity.servicotecnico.servicotecnico, 'id': entity.servicotecnicochecklist }, 
                true, this.rotaErpApi);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate(
                'servicostecnicos_servicostecnicoschecklist_create', 
                angular.extend(this.constructors), 
                true, this.rotaErpApi);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean = false) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.servicotecnicochecklist) {
                    entity.servicotecnicochecklist = response.data.servicotecnicochecklist;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('servicostecnicos_servicostecnicoschecklist_submitted', {
                    entity: entity,
                    response: response,
                    autosave: autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('servicostecnicos_servicostecnicoschecklist_submit_error', {
                    entity: entity,
                    response: response,
                    autosave: autosave
                });
    ...
    get(servicotecnico: any, identifier: string) {
        this.loading_deferred = this.$q.defer();

        return this.$q((resolve: any, reject: any) => {
            this.$http
                .get(this.nsjRouting.generate(
                    'servicostecnicos_servicostecnicoschecklist_get', 
                    { 'servicotecnico': servicotecnico, 'id': identifier }, 
                    true, this.rotaErpApi),
                    { timeout: this.loading_deferred.promise })
                .then((response: any) => {
                    this.$rootScope.$broadcast('servicostecnicos_servicostecnicoschecklist_loaded', response.data);
                    resolve(response.data);
                })
                .catch((response: any) => {
                    reject(response);
