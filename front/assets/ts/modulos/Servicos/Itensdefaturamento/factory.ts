...
export class ServicosItensdefaturamento {
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('servicos_itensdefaturamento_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true, true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.servico = this.entities[this.entities.length - 1].servico;
                        this.after.servico = this.entities[this.entities.length - 1].servico;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('servicos_itensdefaturamento_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].servico === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.servico;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('servicos_itensdefaturamento_delete', angular.extend(this.constructors, {'id': $identifier }), true, true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('servicos_itensdefaturamento_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('servicos_itensdefaturamento_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        ...
        if (entity.servico) {
            method = 'PUT';
            url = this.nsjRouting.generate('servicos_itensdefaturamento_put', {  'id': entity.servico }, true, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('servicos_itensdefaturamento_create', angular.extend(this.constructors), true, true);
        }
        if (!autosave) {
            autosave = false;
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.servico) {
                    entity.servico = response.data.servico;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('servicos_itensdefaturamento_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('servicos_itensdefaturamento_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('servicos_itensdefaturamento_get', {  'id': identifier }, true, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('servicos_itensdefaturamento_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
