...
export class ServicosItensdefaturamentoListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'ServicosItensdefaturamento', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['bloqueado', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'servico') {
        ...
        this.fields = $rootScope.FIELDS_ServicosItensdefaturamento;
                $scope.$on('servicos_itensdefaturamento_deleted', (event: any) => {
        ...
        $rootScope.$on('servicos_itensdefaturamento_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
