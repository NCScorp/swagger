...
export class NsCidadesestrangeirasListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'NsCidadesestrangeiras', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['nome', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'cidadeestrangeira') {
        ...
        this.fields = $rootScope.FIELDS_NsCidadesestrangeiras;
                $scope.$on('ns_cidadesestrangeiras_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_cidadesestrangeiras_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
