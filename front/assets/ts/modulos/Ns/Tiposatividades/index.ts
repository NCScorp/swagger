...
export class NsTiposatividadesListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'tipoatividade') {
        ...
        this.fields = $rootScope.FIELDS_NsTiposatividades;
                $scope.$on('ns_tiposatividades_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_tiposatividades_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
