...
export class NsTiposlogradourosListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'NsTiposlogradouros', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['tipologradouro', 'descricao', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'tipologradouro') {
        ...
        this.fields = $rootScope.FIELDS_NsTiposlogradouros;
                $scope.$on('ns_tiposlogradouros_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_tiposlogradouros_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
