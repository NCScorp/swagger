...
export class NsPaisesListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'NsPaises', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['nome', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'pais') {
        ...
        this.fields = $rootScope.FIELDS_NsPaises;
                $scope.$on('ns_paises_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_paises_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
