...
export class NsMunicipiosListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'NsMunicipios', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['nome', 'uf', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'codigo') {
        ...
        this.fields = $rootScope.FIELDS_NsMunicipios;
                $scope.$on('ns_municipios_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_municipios_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
