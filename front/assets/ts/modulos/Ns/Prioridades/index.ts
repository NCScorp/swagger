...
export class NsPrioridadesListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'prioridade') {
        ...
        this.fields = $rootScope.FIELDS_NsPrioridades;
                $scope.$on('ns_prioridades_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_prioridades_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
