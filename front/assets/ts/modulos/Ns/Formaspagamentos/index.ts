...
export class NsFormaspagamentosListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'NsFormaspagamentos', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['bloqueada', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'formapagamento') {
        ...
        this.fields = $rootScope.FIELDS_NsFormaspagamentos;
                $scope.$on('ns_formaspagamentos_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_formaspagamentos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
