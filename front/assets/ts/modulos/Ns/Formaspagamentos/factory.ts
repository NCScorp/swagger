...
export class NsFormaspagamentos {
    ...
    reload() {
        ...
        delete this.filters['formapagamento'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('ns_formaspagamentos_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true, true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.formapagamento = this.entities[this.entities.length - 1].formapagamento;
                        this.after.formapagamento = this.entities[this.entities.length - 1].formapagamento;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('ns_formaspagamentos_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].formapagamento === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.formapagamento;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('ns_formaspagamentos_delete', angular.extend(this.constructors, {'id': $identifier }), true, true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('ns_formaspagamentos_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('ns_formaspagamentos_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.formapagamento) {
            method = 'PUT';
            url = this.nsjRouting.generate('ns_formaspagamentos_put', {  'id': entity.formapagamento }, true, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('ns_formaspagamentos_create', angular.extend(this.constructors), true, true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.formapagamento) {
                    entity.formapagamento = response.data.formapagamento;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('ns_formaspagamentos_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('ns_formaspagamentos_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('ns_formaspagamentos_get', {  'id': identifier }, true, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('ns_formaspagamentos_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
