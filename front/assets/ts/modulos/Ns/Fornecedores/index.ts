...
export class NsFornecedoresListController {
    static $inject = [
        'utilService',
        '$scope',
        '$stateParams',
    ...
    constructor(
        ...
        public toaster: any,
        public tiposatividadesfilterFilter: any,
        ...
        for (let i in $stateParams) {
            if (['tiposatividadesfilter', 'status', 'municipionome',].indexOf(i) > -1 && $stateParams[i] !== undefined) {
        ...
        this.fields = $rootScope.FIELDS_NsFornecedores;
        $scope.$on('ns_fornecedores_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_fornecedores_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
