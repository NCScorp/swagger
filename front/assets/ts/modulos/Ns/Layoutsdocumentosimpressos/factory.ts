...
export class NsLayoutsdocumentosimpressos {
    ...
    reload() {
        ...
        delete this.filters['layoutdocumentoimpresso'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('ns_layoutsdocumentosimpressos_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.layoutdocumentoimpresso = this.entities[this.entities.length - 1].layoutdocumentoimpresso;
                        this.after.layoutdocumentoimpresso = this.entities[this.entities.length - 1].layoutdocumentoimpresso;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('ns_layoutsdocumentosimpressos_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].layoutdocumentoimpresso === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.layoutdocumentoimpresso;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('ns_layoutsdocumentosimpressos_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('ns_layoutsdocumentosimpressos_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('ns_layoutsdocumentosimpressos_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.layoutdocumentoimpresso) {
            method = 'PUT';
            url = this.nsjRouting.generate('ns_layoutsdocumentosimpressos_put', {  'id': entity.layoutdocumentoimpresso }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('ns_layoutsdocumentosimpressos_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.layoutdocumentoimpresso) {
                    entity.layoutdocumentoimpresso = response.data.layoutdocumentoimpresso;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('ns_layoutsdocumentosimpressos_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('ns_layoutsdocumentosimpressos_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('ns_layoutsdocumentosimpressos_get', {  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('ns_layoutsdocumentosimpressos_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
