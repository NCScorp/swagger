...
export class NsLayoutsdocumentosimpressosListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'NsLayoutsdocumentosimpressos', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['sistema', 'nomedocumento', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'layoutdocumentoimpresso') {
        ...
        this.fields = $rootScope.FIELDS_NsLayoutsdocumentosimpressos;
                $scope.$on('ns_layoutsdocumentosimpressos_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_layoutsdocumentosimpressos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
