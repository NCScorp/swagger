...
export class NsCamposcustomizados {
    ...
    reload() {
        ...
        delete this.filters['campocustomizado'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('ns_camposcustomizados_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    if (data.length > 0) {
                        ...
                        this.$rootScope.$broadcast('ns_camposcustomizados_list_finished', this.entities);
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].campocustomizado === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.campocustomizado;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('ns_camposcustomizados_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('ns_camposcustomizados_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('ns_camposcustomizados_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.campocustomizado) {
            method = 'PUT';
            url = this.nsjRouting.generate('ns_camposcustomizados_put', {  'id': entity.campocustomizado }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('ns_camposcustomizados_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.campocustomizado) {
                    entity.campocustomizado = response.data.campocustomizado;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('ns_camposcustomizados_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('ns_camposcustomizados_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('ns_camposcustomizados_get', {  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('ns_camposcustomizados_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
    ...
    /**
     * Busca lista de todos os campos customizados
     * @param filtros 
     */
    public getAll(filtros: any = {}, erpApi: boolean = false): Promise<any[]> {
        return new Promise((resolve, reject) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('ns_camposcustomizados_index', filtros, true, erpApi)
            }).then((response: any) => {
                resolve(response.data);
                this.$rootScope.$broadcast('ns_camposcustomizados_list_finished', response.data);
            }).catch((erro: any) => {
                reject(erro);
            })
        });
