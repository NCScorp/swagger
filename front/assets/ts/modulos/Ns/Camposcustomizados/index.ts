...
export class NsCamposcustomizadosListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'NsCamposcustomizados', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['crmnegocio', 'crmnegociosvisualizacao', 'crmnegocioexibenalistagem', 'secao', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'campocustomizado') {
        ...
        this.fields = $rootScope.FIELDS_NsCamposcustomizados;
                $scope.$on('ns_camposcustomizados_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_camposcustomizados_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
