...
export class NsTelefones {
    ...
    reload() {
        ...
        delete this.filters['telefone_id'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('ns_telefones_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.telefone_id = this.entities[this.entities.length - 1].telefone_id;
                        this.after.telefone_id = this.entities[this.entities.length - 1].telefone_id;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('ns_telefones_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].telefone_id === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.telefone_id;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('ns_telefones_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('ns_telefones_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('ns_telefones_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.telefone_id) {
            method = 'PUT';
            url = this.nsjRouting.generate('ns_telefones_put', { contato : entity.contato,  'id': entity.telefone_id }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('ns_telefones_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.telefone_id) {
                    entity.telefone_id = response.data.telefone_id;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('ns_telefones_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('ns_telefones_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(contato: any, identifier : string) {
                
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('ns_telefones_get', { 'contato': contato,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('ns_telefones_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
