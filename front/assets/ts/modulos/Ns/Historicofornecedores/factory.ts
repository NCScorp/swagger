...
export class NsHistoricofornecedores {
    ...
    reload() {
        ...
        delete this.filters['historicofornecedor'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('ns_historicofornecedores_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    if (data.length > 0) {
                        ...
                        this.after.historicofornecedor = this.entities[this.entities.length - 1].historicofornecedor;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('ns_historicofornecedores_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].historicofornecedor === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.historicofornecedor;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('ns_historicofornecedores_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('ns_historicofornecedores_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('ns_historicofornecedores_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.historicofornecedor) {
            method = 'PUT';
            url = this.nsjRouting.generate('ns_historicofornecedores_put', { fornecedor : entity.fornecedor,  'id': entity.historicofornecedor }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('ns_historicofornecedores_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.historicofornecedor) {
                    entity.historicofornecedor = response.data.historicofornecedor;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('ns_historicofornecedores_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('ns_historicofornecedores_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(fornecedor: any, identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('ns_historicofornecedores_get', { 'fornecedor': fornecedor,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('ns_historicofornecedores_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
