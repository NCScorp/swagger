...
export class NsPessoastiposatividades {
    ...
    reload() {
        ...
        delete this.filters['pessoatipoatividade'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('ns_pessoastiposatividades_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.pessoatipoatividade = this.entities[this.entities.length - 1].pessoatipoatividade;
                        this.after.pessoatipoatividade = this.entities[this.entities.length - 1].pessoatipoatividade;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('ns_pessoastiposatividades_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].pessoatipoatividade === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.pessoatipoatividade;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('ns_pessoastiposatividades_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('ns_pessoastiposatividades_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('ns_pessoastiposatividades_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.pessoatipoatividade) {
            method = 'PUT';
            url = this.nsjRouting.generate('ns_pessoastiposatividades_put', { pessoa : entity.pessoa,  'id': entity.pessoatipoatividade }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('ns_pessoastiposatividades_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.pessoatipoatividade) {
                    entity.pessoatipoatividade = response.data.pessoatipoatividade;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('ns_pessoastiposatividades_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('ns_pessoastiposatividades_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(pessoa: any, identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('ns_pessoastiposatividades_get', { 'pessoa': pessoa,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('ns_pessoastiposatividades_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
