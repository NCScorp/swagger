...
export class NsFornecedoressuspensosListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'NsFornecedoressuspensos', 'toaster'
    ,
                    'fornecedoresfilterFilter',                    '$rootScope', '$location', 'nsjRouting'];
    ...
    ,
                public fornecedoresfilterFilter: any,                public $rootScope: any, public $location: any, public nsjRouting: any ) {
        entityService.filter =  $stateParams.q ? $stateParams.q : '';
        entityService.filters = {};
        entityService.constructors = {};
        for (let i in $stateParams) {
            if (  ['fornecedoresfilter', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'fornecedorsuspenso') {
        ...
        this.fields = $rootScope.FIELDS_NsFornecedoressuspensos;
                $scope.$on('ns_fornecedoressuspensos_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_fornecedoressuspensos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
