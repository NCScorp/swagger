...
export class NsFornecedoressuspensos {
    ...
    reload() {
        ...
        delete this.filters['fornecedorsuspenso'];
        return this.load();
    }
            ...
            for ( let fil in filter.filters ) {
                if (typeof(filter.filters[fil]) !== 'object') {
                    ...
                    filters[fil] = filter.filters[fil];
                        ...
                        if ( filter.filterLookup && filter.filterLookup.fornecedoresfilter ) {
                filters.fornecedoresfilter = filter.filterLookup.fornecedoresfilter.fornecedor;
            }
                    } else if (filter == null) {
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('ns_fornecedoressuspensos_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true, true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.fornecedorsuspenso = this.entities[this.entities.length - 1].fornecedorsuspenso;
                        this.after.fornecedorsuspenso = this.entities[this.entities.length - 1].fornecedorsuspenso;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('ns_fornecedoressuspensos_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].fornecedorsuspenso === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.fornecedorsuspenso;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('ns_fornecedoressuspensos_delete', angular.extend(this.constructors, {'id': $identifier }), true, true))
                .then((response: any) => {
                ...
                .catch((response: any) => {
                    this.$rootScope.$broadcast('ns_fornecedoressuspensos_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.fornecedorsuspenso) {
            method = 'PUT';
            url = this.nsjRouting.generate('ns_fornecedoressuspensos_put', {  'id': entity.fornecedorsuspenso }, true, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('ns_fornecedoressuspensos_create', angular.extend(this.constructors), true, true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.fornecedorsuspenso) {
                    entity.fornecedorsuspenso = response.data.fornecedorsuspenso;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('ns_fornecedoressuspensos_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('ns_fornecedoressuspensos_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('ns_fornecedoressuspensos_reativar', {  'id': entity.fornecedorsuspenso }, true, true),
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('ns_fornecedoressuspensos_get', {  'id': identifier }, true, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('ns_fornecedoressuspensos_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
