import angular = require('angular');
export class NsFornecedoressuspensosShowShowController {

    static $inject = ['$scope', 'toaster', 'NsFornecedoressuspensos', 'utilService'];

    public entity: any;
    public form: any;
    public constructors: any;
    public collection: any;
    public busy: boolean;
    public action: string;
    public idCount: number = 0;

    constructor(public $scope: angular.IScope, public toaster: any, public entityService: any, public utilService: any) {
    }

    $onInit() {
        this.superOnInit();
        if (this.entity.fornecedores != undefined || this.entity.fornecedores != null) {
            this.entity.fornecedores.cnpj = this.entity.fornecedores.cnpj.replace(/[^\d]+/g, '');
        }
    }
    superOnInit() {
        this.$scope.$watch('$ctrl.entity', (newValue, oldValue) => {
            if (newValue !== oldValue) {
                this.form.$setDirty();
            }
        }, true);
    }
    uniqueValidation(params: any): any {
        if (this.collection) {
            let validation = { 'unique': true };
            for (var item in this.collection) {
                if (this.collection[item][params.field] === params.value) {
                    validation.unique = false;
                    break;
                }
            }
            return validation;
        } else {
            return null;
        }

    }
}