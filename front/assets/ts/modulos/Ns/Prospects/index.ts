...
export class NsProspectsListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'prospect') {
        ...
        this.fields = $rootScope.FIELDS_NsProspects;
                $scope.$on('ns_prospects_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_prospects_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
