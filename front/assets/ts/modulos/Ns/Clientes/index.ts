...
export class NsClientesListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'NsClientes', 'toaster'
    ,
                    'tiposatividadesfilterFilter',                    '$rootScope', '$location', 'nsjRouting'];
    ...
    ,
                public tiposatividadesfilterFilter: any,                public $rootScope: any, public $location: any, public nsjRouting: any ) {
        entityService.filter =  $stateParams.q ? $stateParams.q : '';
        entityService.filters = {};
        entityService.constructors = {};
        for (let i in $stateParams) {
            if (  ['tipo', 'tiposatividadesfilter', 'cnpj', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'cliente') {
        ...
        this.fields = $rootScope.FIELDS_NsClientes;
                $scope.$on('ns_clientes_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_clientes_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
