...
export class NsDocumentosfopListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'NsDocumentosfop', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'documentofop') {
        ...
        this.fields = $rootScope.FIELDS_NsDocumentosfop;
                $scope.$on('ns_documentosfop_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_documentosfop_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
