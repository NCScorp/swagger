...
export class NsCidadesinformacoesfunerariasListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['municipio', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'cidadeinformacaofuneraria') {
        ...
        this.fields = $rootScope.FIELDS_NsCidadesinformacoesfunerarias;
                $scope.$on('ns_cidadesinformacoesfunerarias_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_cidadesinformacoesfunerarias_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
