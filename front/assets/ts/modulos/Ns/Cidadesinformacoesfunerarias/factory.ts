...
export class NsCidadesinformacoesfunerarias {
    ...
    reload() {
        ...
        delete this.filters['cidadeinformacaofuneraria'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('ns_cidadesinformacoesfunerarias_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true, true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.cidadeinformacaofuneraria = this.entities[this.entities.length - 1].cidadeinformacaofuneraria;
                        this.after.cidadeinformacaofuneraria = this.entities[this.entities.length - 1].cidadeinformacaofuneraria;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('ns_cidadesinformacoesfunerarias_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].cidadeinformacaofuneraria === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.cidadeinformacaofuneraria;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('ns_cidadesinformacoesfunerarias_delete', angular.extend(this.constructors, {'id': $identifier }), true, true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('ns_cidadesinformacoesfunerarias_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('ns_cidadesinformacoesfunerarias_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.cidadeinformacaofuneraria) {
            method = 'PUT';
            url = this.nsjRouting.generate('ns_cidadesinformacoesfunerarias_put', {  'id': entity.cidadeinformacaofuneraria }, true, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('ns_cidadesinformacoesfunerarias_create', angular.extend(this.constructors), true, true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.cidadeinformacaofuneraria) {
                    entity.cidadeinformacaofuneraria = response.data.cidadeinformacaofuneraria;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('ns_cidadesinformacoesfunerarias_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('ns_cidadesinformacoesfunerarias_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('ns_cidadesinformacoesfunerarias_get', {  'id': identifier }, true, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('ns_cidadesinformacoesfunerarias_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
