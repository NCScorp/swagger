...
export class NsDocumentosnecessarios {
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('ns_documentosnecessarios_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    if (data.length > 0) {
                        ...
                        this.$rootScope.$broadcast('ns_documentosnecessarios_list_finished', this.entities);
                                        this.loaded = true;
    ...
    delete($identifier: any, force: boolean) {
        ...
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('ns_documentosnecessarios_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('ns_documentosnecessarios_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('ns_documentosnecessarios_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        ...
        if (entity.id) {
            method = 'PUT';
            url = this.nsjRouting.generate('ns_documentosnecessarios_put', { tipodocumento : entity.tipodocumento,  'id': entity.id }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('ns_documentosnecessarios_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                ...
                entity.$$_saved = true;
                this.$rootScope.$broadcast('ns_documentosnecessarios_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('ns_documentosnecessarios_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(tipodocumento: any, identifier : string) {
                
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('ns_documentosnecessarios_get', { 'tipodocumento': tipodocumento,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('ns_documentosnecessarios_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
