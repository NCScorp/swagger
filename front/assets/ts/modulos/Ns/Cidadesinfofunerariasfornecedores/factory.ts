...
export class NsCidadesinfofunerariasfornecedores {
    ...
    reload() {
        ...
        delete this.filters['cidadeinfofunerariafornecedor'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('ns_cidadesinfofunerariasfornecedores_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.cidadeinfofunerariafornecedor = this.entities[this.entities.length - 1].cidadeinfofunerariafornecedor;
                        this.after.cidadeinfofunerariafornecedor = this.entities[this.entities.length - 1].cidadeinfofunerariafornecedor;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('ns_cidadesinfofunerariasfornecedores_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].cidadeinfofunerariafornecedor === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.cidadeinfofunerariafornecedor;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('ns_cidadesinfofunerariasfornecedores_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('ns_cidadesinfofunerariasfornecedores_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('ns_cidadesinfofunerariasfornecedores_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.cidadeinfofunerariafornecedor) {
            method = 'PUT';
            url = this.nsjRouting.generate('ns_cidadesinfofunerariasfornecedores_put', { cidadeinformacaofuneraria : entity.cidadeinformacaofuneraria,  'id': entity.cidadeinfofunerariafornecedor }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('ns_cidadesinfofunerariasfornecedores_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.cidadeinfofunerariafornecedor) {
                    entity.cidadeinfofunerariafornecedor = response.data.cidadeinfofunerariafornecedor;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('ns_cidadesinfofunerariasfornecedores_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('ns_cidadesinfofunerariasfornecedores_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(cidadeinformacaofuneraria: any, identifier : string) {
                
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('ns_cidadesinfofunerariasfornecedores_get', { 'cidadeinformacaofuneraria': cidadeinformacaofuneraria,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('ns_cidadesinfofunerariasfornecedores_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
