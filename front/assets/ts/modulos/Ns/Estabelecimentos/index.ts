...
export class NsEstabelecimentosListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'NsEstabelecimentos', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'estabelecimento') {
        ...
        this.fields = $rootScope.FIELDS_NsEstabelecimentos;
                $scope.$on('ns_estabelecimentos_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_estabelecimentos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
