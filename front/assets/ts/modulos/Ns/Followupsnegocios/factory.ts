...
export class NsFollowupsnegocios {
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('ns_followupsnegocios_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    if (data.length > 0) {
                        ...
                        this.$rootScope.$broadcast('ns_followupsnegocios_list_finished', this.entities);
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].followup === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        ...
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('ns_followupsnegocios_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('ns_followupsnegocios_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('ns_followupsnegocios_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        ...
        if (entity.followup) {
            method = 'PUT';
            url = this.nsjRouting.generate('ns_followupsnegocios_put', { proposta : entity.proposta.documento,  'id': entity.followup }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('ns_followupsnegocios_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.followup) {
                    entity.followup = response.data.followup;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('ns_followupsnegocios_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('ns_followupsnegocios_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(proposta: any, identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('ns_followupsnegocios_get', { 'proposta': proposta,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('ns_followupsnegocios_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
