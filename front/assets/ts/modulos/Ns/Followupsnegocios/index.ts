...
export class NsFollowupsnegociosListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'followup') {
        ...
        this.fields = $rootScope.FIELDS_NsFollowupsnegocios;
                $scope.$on('ns_followupsnegocios_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_followupsnegocios_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
