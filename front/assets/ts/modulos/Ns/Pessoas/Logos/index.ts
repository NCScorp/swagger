...
export class NsPessoasLogosListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'pessoalogo') {
        ...
        this.fields = $rootScope.FIELDS_NsPessoasLogos;
                $scope.$on('ns_pessoas_logos_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_pessoas_logos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
