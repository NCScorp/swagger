...
export class NsPessoasLogos {
    ...
    reload() {
        ...
        delete this.filters['pessoalogo'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('ns_pessoas_logos_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    if (data.length > 0) {
                        ...
                        this.$rootScope.$broadcast('ns_pessoas_logos_list_finished', this.entities);
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].pessoalogo === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.pessoalogo;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('ns_pessoas_logos_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('ns_pessoas_logos_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('ns_pessoas_logos_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.pessoalogo) {
            method = 'PUT';
            url = this.nsjRouting.generate('ns_pessoas_logos_put', { idpessoa : entity.idpessoa,  'id': entity.pessoalogo }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('ns_pessoas_logos_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.pessoalogo) {
                    entity.pessoalogo = response.data.pessoalogo;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('ns_pessoas_logos_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('ns_pessoas_logos_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(idpessoa: any, identifier : string) {
                
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('ns_pessoas_logos_get', { 'idpessoa': idpessoa,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('ns_pessoas_logos_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
