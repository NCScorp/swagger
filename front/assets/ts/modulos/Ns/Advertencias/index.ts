...
export class NsAdvertenciasListController {
    static $inject = ['utilService', '$scope', '$stateParams', '$state', 'NsAdvertencias', 'toaster'
        ,
        'fornecedoresFilter', '$rootScope', '$location', 'nsjRouting'];
    ...
    public filtrofornecedor: any;

    constructor(public utilService: any, public $scope: any, public $stateParams: any, public $state: any, public entityService: any, public toaster: any
        ,
        public fornecedoresFilter: any, public $rootScope: any, public $location: any, public nsjRouting: any) {
        entityService.filter = $stateParams.q ? $stateParams.q : '';
        entityService.filters = {};
        entityService.constructors = {};
        for (let i in $stateParams) {
            if (['fornecedores',].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
            } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'advertencia') {
        ...
        this.fields = $rootScope.FIELDS_NsAdvertencias;
        $scope.$on('ns_advertencias_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_advertencias_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
        this.filtrofornecedor = this.fornecedoresFilter == null || this.fornecedoresFilter.fornecedor == null ? false : true;
    }

    search(filter: any) {

        /* Sobrescrito para verificar filtro de fornecedores */
        this.filtrofornecedor = this.fornecedoresFilter == null || this.fornecedoresFilter.fornecedor == null ? false : true;
        /* --- */
