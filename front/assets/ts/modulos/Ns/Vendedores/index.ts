...
export class NsVendedoresListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'NsVendedores', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['vendedor_bloqueado', 'vendedoremail', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'vendedor_id') {
        ...
        this.fields = $rootScope.FIELDS_NsVendedores;
                $scope.$on('ns_vendedores_deleted', (event: any) => {
        ...
        $rootScope.$on('ns_vendedores_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
