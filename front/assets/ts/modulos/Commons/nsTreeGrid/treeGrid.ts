        ...
        .run([
            ...
            function ($templateCache) {
                $templateCache.put('template/treeGrid/treeGrid.html',
                    "<div style=\"width:100%\" >\n" +
                    " <table class=\"table tree-grid nsj-table btns-actions\">\n" +
                    "   <thead>\n" +
                    "     <tr>\n" +
                    "       <th class='expand-property' ng-if=\"!campoExpansao\"><span ng-if=\"expandingProperty.sortable\" ng-click=\"sortBy(expandingProperty)\">{{expandingProperty.displayName || expandingProperty.field || expandingProperty}}</span><span ng-if=\"!expandingProperty.sortable\">{{expandingProperty.displayName || expandingProperty.field || expandingProperty}}</span><nsj-button ng-if=\"expandingProperty.sorted\" color=\"link\" icon=\"{{expandingProperty.sortingIcon}}\" class=\"pull-right\"></nsj-button></th>\n" +
                    "       <th ng-repeat=\"col in colDefinitions\" class=\"{{col.field}}\"><a ng-if=\"col.sortable\" ng-click=\"sortBy(col)\">{{col.displayName || col.field}}</a><span ng-if=\"!col.sortable\">{{col.displayName || col.field}}</span><i ng-if=\"col.sorted\" class=\"{{col.sortingIcon}} pull-right\"></i></th>\n" +
                    "     </tr>\n" +
                    "   </thead>\n" +
                    "   <tbody>\n" +
                    "     <tr ng-repeat=\"row in tree_rows | searchFor:$parent.filterString:expandingProperty:colDefinitions track by row.branch.uid\"\n" +
                    "       ng-class=\"'level-' + {{ row.level }} + (row.branch.selected ? ' active':'') \" class=\"tree-grid-row {{ row.classes }}\">\n" +
                    "       <td ng-if=\"!campoExpansao\"><a ng-click=\"user_clicks_branch(row.branch);\"><nsj-button color=\"link\" icon=\"{{row.tree_icon}}\"\n" +
                    "              " +
                    "              class=\"indented tree-icon\"" + " ng-click=\"row.branch.expanded = !row.branch.expanded\"></nsj-button></a><span ng-if=\"expandingProperty.cellTemplate\" cell-template-scope=\"expandingProperty.cellTemplateScope\" class=\"indented tree-label\" " +
                    "              ng-click=\"on_user_click(row.branch)\" compile-template=\"expandingProperty.cellTemplate\"></span>" +
                    "              <span  ng-if=\"!expandingProperty.cellTemplate\" class=\"indented tree-label\" ng-click=\"on_user_click(row.branch)\">\n" +
                    "             {{row.branch[expandingProperty.field] || row.branch[expandingProperty]}}</span>\n" +
                    "       </td>\n" +
                    "       <td ng-repeat=\"col in colDefinitions\" class=\"{{col.field}}\" >\n" +
                    "         <a ng-click=\"user_clicks_branch(row.branch);\" ng-if=\"campoExpansao && campoExpansao === col.field \" > \n" +
                    "           <nsj-button \n" +
                    "             color=\"link\" \n" +
                    "             icon=\"{{row.tree_icon}}\" \n" +
                    "             class=\"indented tree-icon btn-expansao\" \n" +
                    "             ng-click=\"row.branch.expanded = !row.branch.expanded\" \n" +
                    "           > \n" +
                    "           </nsj-button> \n" +
                    "         </a> \n" +
                    "         <div ng-if=\"col.cellTemplate\" class=\"tree-coluna-container\" compile-template=\"col.cellTemplate\" cell-template-scope=\"col.cellTemplateScope\"></div>\n" +
                    "         <div ng-if=\"!col.cellTemplate\" class=\"tree-coluna-container\" >{{row.branch[col.field]}}</div>\n" +
                    "       </td>\n" +
                    "     </tr>\n" +
                    "   </tbody>\n" +
                    " </table>\n" +
            ...
            function ($compile) {
                return {
                    ...
                    link: function (scope:any , element:any, attrs:any) {
                        ...
                        scope.$watch(attrs.compileTemplate, function (new_val) {
                            /*
                ...
                return {
                    ...
                    link: function (scope:any, element:any, attrs:any) {
                        ...
                        var sort_by = function (col, descending) {
                            ...
                            return function (a, b) {
                                return a = key(a), b = key(b), direction * (<any>(a > b) - <any>(b > a));
                            };
                        }
                        ...
                        on_treeData_change = function () {
                            ...
                            add_branch_to_list = function (level, branch, visible) {
                                ...
                                branch.level = level;
                                if (scope.getRowClass()){
                                    getRowClass = scope.getRowClass();
                                }
...
export const treeGrid =  treeGridName
