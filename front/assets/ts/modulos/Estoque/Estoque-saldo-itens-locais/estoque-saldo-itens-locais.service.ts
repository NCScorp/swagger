...
export class EstoqueSaldoItensLocaisService {
    ...
    reload() {
        ...
        delete this.filters['localdeestoque'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('estoque_saldoitenslocaisdeestoques_index', angular.extend({}, constructors, { 'offset': offset, 'filter': search }, filters ? filters : this.filters), true, this.rotaErpApi),
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    if (data.length > 0) {
                        ...
                        this.after.localdeestoque = this.entities[this.entities.length - 1].localdeestoque;
                        this.after.localdeestoque = this.entities[this.entities.length - 1].localdeestoque;
                    }
                    if (data.length < 20) {
                        this.loadParams.finished = true;
                        this.$rootScope.$broadcast('estoque_saldoitenslocaisdeestoques_list_finished', this.entities);
                    } else {
                        this.loadParams.finished = false;
                    }
                    this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].localdeestoque === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof ($identifier) === 'object') {
            $identifier = $identifier.localdeestoque;
        }
        if ($identifier) {
            if (force) {
                this.loading_deferred = this.$q.defer();
                this.$http
                    .delete(this.nsjRouting.generate('estoque_saldoitenslocaisdeestoques_delete', angular.extend(this.constructors, { 'id': $identifier }), true, this.rotaErpApi),
                        { timeout: this.loading_deferred.promise })
                    .then((response: any) => {
                        this.$rootScope.$broadcast('estoque_saldoitenslocaisdeestoques_deleted', {
                            entity: this.entity,
                            response: response
                        });
                    })
                    .catch((response: any) => {
                        this.$rootScope.$broadcast('estoque_saldoitenslocaisdeestoques_delete_error', {
                            entity: this.entity,
                            response: response
                        });
                    });
    ...
    _save(entity: any, autosave: boolean = false) {
        let method, url;
        if (entity.localdeestoque) {
            method = 'PUT';
            url = this.nsjRouting.generate('estoque_saldoitenslocaisdeestoques_put', { 'id': entity.localdeestoque }, true, this.rotaErpApi);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('estoque_saldoitenslocaisdeestoques_create', angular.extend(this.constructors), true, this.rotaErpApi);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean = false) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.localdeestoque) {
                    entity.localdeestoque = response.data.localdeestoque;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('estoque_saldoitenslocaisdeestoques_submitted', {
                    entity: entity,
                    response: response,
                    autosave: autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('estoque_saldoitenslocaisdeestoques_submit_error', {
                    entity: entity,
                    response: response,
                    autosave: autosave
                });
    ...
    get(identifier: string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
                .get(this.nsjRouting.generate('estoque_saldoitenslocaisdeestoques_get', { 'id': identifier }, true, this.rotaErpApi),
                    { timeout: this.loading_deferred.promise })
                .then((response: any) => {
                    this.$rootScope.$broadcast('estoque_saldoitenslocaisdeestoques_loaded', response.data);
                    resolve(response.data);
                })
                .catch((response: any) => {
                    reject(response);
