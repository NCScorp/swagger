...
export class EstoqueUnidadesListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'unidade') {
        ...
        this.fields = $rootScope.FIELDS_EstoqueUnidades;
                $scope.$on('estoque_unidades_deleted', (event: any) => {
        ...
        $rootScope.$on('estoque_unidades_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
