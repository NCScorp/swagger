...
export class EstoqueFamiliasListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'EstoqueFamilias', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'familia') {
        ...
        this.fields = $rootScope.FIELDS_EstoqueFamilias;
                $scope.$on('estoque_familias_deleted', (event: any) => {
        ...
        $rootScope.$on('estoque_familias_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
