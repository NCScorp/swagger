...
export class FinancasContasListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'FinancasContas', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['bloqueado', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'conta') {
        ...
        this.fields = $rootScope.FIELDS_FinancasContas;
                $scope.$on('financas_contas_deleted', (event: any) => {
        ...
        $rootScope.$on('financas_contas_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
