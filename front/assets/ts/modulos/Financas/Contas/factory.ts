...
export class FinancasContas {
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('financas_contas_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true,true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.conta = this.entities[this.entities.length - 1].conta;
                        this.after.conta = this.entities[this.entities.length - 1].conta;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('financas_contas_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].conta === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.conta;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('financas_contas_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('financas_contas_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('financas_contas_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        ...
        if (entity.conta) {
            method = 'PUT';
            url = this.nsjRouting.generate('financas_contas_put', {  'id': entity.conta }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('financas_contas_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.conta) {
                    entity.conta = response.data.conta;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('financas_contas_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('financas_contas_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('financas_contas_get', {  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('financas_contas_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
