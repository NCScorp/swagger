...
export class FinancasItenscontratos {
    ...
    reload() {
        ...
        delete this.filters['itemcontrato'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('financas_itenscontratos_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.itemcontrato = this.entities[this.entities.length - 1].itemcontrato;
                        this.after.itemcontrato = this.entities[this.entities.length - 1].itemcontrato;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('financas_itenscontratos_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].itemcontrato === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.itemcontrato;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('financas_itenscontratos_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('financas_itenscontratos_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('financas_itenscontratos_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.itemcontrato) {
            method = 'PUT';
            url = this.nsjRouting.generate('financas_itenscontratos_put', {  'id': entity.itemcontrato }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('financas_itenscontratos_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.itemcontrato) {
                    entity.itemcontrato = response.data.itemcontrato;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('financas_itenscontratos_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('financas_itenscontratos_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('financas_itenscontratos_get', {  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('financas_itenscontratos_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
