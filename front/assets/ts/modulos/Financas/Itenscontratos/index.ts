...
export class FinancasItenscontratosListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'FinancasItenscontratos', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'itemcontrato') {
        ...
        this.fields = $rootScope.FIELDS_FinancasItenscontratos;
                $scope.$on('financas_itenscontratos_deleted', (event: any) => {
        ...
        $rootScope.$on('financas_itenscontratos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
