...
export class FinancasContratosListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'FinancasContratos', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'contrato') {
        ...
        this.fields = $rootScope.FIELDS_FinancasContratos;
                $scope.$on('financas_contratos_deleted', (event: any) => {
        ...
        $rootScope.$on('financas_contratos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
