...
export class FinancasContratos {
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('financas_contratos_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.contrato = this.entities[this.entities.length - 1].contrato;
                        this.after.contrato = this.entities[this.entities.length - 1].contrato;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('financas_contratos_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].contrato === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.contrato;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('financas_contratos_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('financas_contratos_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('financas_contratos_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        ...
        if (entity.contrato) {
            method = 'PUT';
            url = this.nsjRouting.generate('financas_contratos_put', {  'id': entity.contrato }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('financas_contratos_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.contrato) {
                    entity.contrato = response.data.contrato;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('financas_contratos_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('financas_contratos_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
        ...
        excluiContratoPagamento(entity: any) {     
        entity.$$__submitting = true;
        return this.$q((resolve: any, reject: any) => {
            this.$http({
            method: 'post',
            url: this.nsjRouting.generate('financas_contratos_exclui_contrato_pagamento', {  'id': entity.contrato }, true),
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('financas_contratos_get', {  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('financas_contratos_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
