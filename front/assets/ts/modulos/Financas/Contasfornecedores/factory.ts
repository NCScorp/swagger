...
export class FinancasContasfornecedores {
    ...
    reload() {
        ...
        delete this.filters['contafornecedor'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('financas_contasfornecedores_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.contafornecedor = this.entities[this.entities.length - 1].contafornecedor;
                        this.after.contafornecedor = this.entities[this.entities.length - 1].contafornecedor;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('financas_contasfornecedores_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].contafornecedor === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.contafornecedor;
        }
        if ($identifier) {
            if (force  || confirm('Tem certeza que deseja deletar?') ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('financas_contasfornecedores_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('financas_contasfornecedores_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('financas_contasfornecedores_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.contafornecedor) {
            method = 'PUT';
            url = this.nsjRouting.generate('financas_contasfornecedores_put', { fornecedor : entity.fornecedor,  'id': entity.contafornecedor }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('financas_contasfornecedores_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.contafornecedor) {
                    entity.contafornecedor = response.data.contafornecedor;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('financas_contasfornecedores_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('financas_contasfornecedores_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(fornecedor: any, identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('financas_contasfornecedores_get', { 'fornecedor': fornecedor,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('financas_contasfornecedores_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
