...
export class FinancasBancosListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'FinancasBancos', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['numero', 'nome', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'banco') {
        ...
        this.fields = $rootScope.FIELDS_FinancasBancos;
                $scope.$on('financas_bancos_deleted', (event: any) => {
        ...
        $rootScope.$on('financas_bancos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
