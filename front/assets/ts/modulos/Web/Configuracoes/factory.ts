...
export class WebConfiguracoes {
    ...
    reload() {
        ...
        delete this.filters['configuracao'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('web_configuracoes_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
                    ...
                    if (data.length > 0) {
                        for (let i = 0; i < data.length; i++) {
                            this.entities.push(data[i]);
                                                ...
                                                this.after.configuracao = this.entities[this.entities.length - 1].configuracao;
                        this.after.configuracao = this.entities[this.entities.length - 1].configuracao;
                                            }
                                        if (data.length < 20) {
                                            this.loadParams.finished = true;
                        this.$rootScope.$broadcast('web_configuracoes_list_finished', this.entities);
                                        } else {
                        this.loadParams.finished = false;
                    }
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].configuracao === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.configuracao;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('web_configuracoes_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('web_configuracoes_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('web_configuracoes_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.configuracao) {
            method = 'PUT';
            url = this.nsjRouting.generate('web_configuracoes_put', {  'id': entity.configuracao }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('web_configuracoes_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.configuracao) {
                    entity.configuracao = response.data.configuracao;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('web_configuracoes_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('web_configuracoes_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(identifier : string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('web_configuracoes_get', {  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('web_configuracoes_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
