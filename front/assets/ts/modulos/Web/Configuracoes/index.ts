...
export class WebConfiguracoesListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  ['chave', 'valor', 'sistema', ].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'configuracao') {
        ...
        this.fields = $rootScope.FIELDS_WebConfiguracoes;
                $scope.$on('web_configuracoes_deleted', (event: any) => {
        ...
        $rootScope.$on('web_configuracoes_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
