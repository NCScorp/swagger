...
export class EtapasService {
    ...
    reload() {
        ...
        delete this.filters['projetoetapa'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('gp_projetosetapas_index', angular.extend({}, constructors, { 'offset': offset, 'filter': search }, filters ? filters : this.filters), true, true),
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    if (data.length > 0) {
                        ...
                        this.after.projetoetapa = this.entities[this.entities.length - 1].projetoetapa;
                        this.after.projetoetapa = this.entities[this.entities.length - 1].projetoetapa;
                    }
                    if (data.length < 20) {
                        this.loadParams.finished = true;
                        this.$rootScope.$broadcast('etapas_list_finished', this.entities);
                    } else {
                        this.loadParams.finished = false;
                    }
                    this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].projetoetapa === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof ($identifier) === 'object') {
            $identifier = $identifier.projetoetapa;
        }
        if ($identifier) {
            if (force || confirm('Tem certeza que deseja deletar?')) {
                this.loading_deferred = this.$q.defer();
                this.$http
                    .delete(this.nsjRouting.generate('gp_projetosetapas_delete', angular.extend(this.constructors, { 'id': $identifier }), true, true),
                        { timeout: this.loading_deferred.promise })
                    .then((response: any) => {
                        this.$rootScope.$broadcast('etapas_deleted', {
                            entity: this.entity,
                            response: response
                        });
                    })
                    .catch((response: any) => {
                        this.$rootScope.$broadcast('etapas_delete_error', {
                            entity: this.entity,
                            response: response
                        });
                    });
    ...
    _save(entity: any, autosave: boolean =  false) {
        let method, url;
        if (entity.projetoetapa) {
            method = 'PUT';
            url = this.nsjRouting.generate('gp_projetosetapas_put', { 'id': entity.projetoetapa }, true, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('gp_projetosetapas_create', angular.extend(this.constructors), true, true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean = false) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.projetoetapa) {
                    entity.projetoetapa = response.data.projetoetapa;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('etapas_submitted', {
                    entity: entity,
                    response: response,
                    autosave: autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('etapas_submit_error', {
                    entity: entity,
                    response: response,
                    autosave: autosave
                });
    ...
    get(identifier: string) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http
                .get(this.nsjRouting.generate('gp_projetosetapas_get', { 'id': identifier }, true, true),
                    { timeout: this.loading_deferred.promise })
                .then((response: any) => {
                    this.$rootScope.$broadcast('etapas_loaded', response.data);
                    resolve(response.data);
                })
                .catch((response: any) => {
                    reject(response);
