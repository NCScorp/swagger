...
export class GpProjetosescopo {
    ...
    reload() {
        ...
        delete this.filters['projetoescopo'];
        return this.load();
    }
    ...
    _load(constructors: any, offset: any, search: string, filters?: any, useFilterPromise?: boolean) {
        ...
        return this.$q((resolve: any, reject: any) => {
            this.$http({
                method: 'GET',
                url: this.nsjRouting.generate('gp_projetosescopo_index', angular.extend({}, constructors, {'offset': offset, 'filter': search}, filters ? filters : this.filters), true),
            ...
            this._load(this.constructors, this.after, this.filter)
                .then((data: any) => {
                    if (data.length > 0) {
                        ...
                        this.$rootScope.$broadcast('gp_projetosescopo_list_finished', this.entities);
                                        this.loaded = true;
    ...
    find(identifier: string) {
        for (let i in this.entities) {
            if (this.entities[i].projetoescopo === identifier) {
    ...
    delete($identifier: any, force: boolean) {
        if (typeof($identifier) === 'object') {
            $identifier = $identifier.projetoescopo;
        }
        if ($identifier) {
            if (force ) {
                this.loading_deferred = this.$q.defer();
                this.$http
                                .delete(this.nsjRouting.generate('gp_projetosescopo_delete', angular.extend(this.constructors, {'id': $identifier }), true))
                .then((response: any) => {
                    this.$rootScope.$broadcast('gp_projetosescopo_deleted', {
                        entity: this.entity,
                        response : response
                    });
                })
                .catch((response: any) => {
                    this.$rootScope.$broadcast('gp_projetosescopo_delete_error', {
                        entity: this.entity,
                        response : response
                    });
                });
    ...
    _save(entity: any, autosave: boolean) {
        let method, url;
        if (entity.projetoescopo) {
            method = 'PUT';
            url = this.nsjRouting.generate('gp_projetosescopo_put', { projeto : entity.projeto.projeto,  'id': entity.projetoescopo }, true);
        } else {
            method = 'POST';
            url = this.nsjRouting.generate('gp_projetosescopo_create', angular.extend(this.constructors), true);
        }
        if (!autosave) {
            autosave = false;
    ...
    save(entity: any, autosave: boolean) {
        ...
        this._save(entity, autosave)
            .then((response: any) => {
                if (response.data.projetoescopo) {
                    entity.projetoescopo = response.data.projetoescopo;
                }
                entity.$$_saved = true;
                this.$rootScope.$broadcast('gp_projetosescopo_submitted', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
            ...
            .catch((response: any) => {
                this.$rootScope.$broadcast('gp_projetosescopo_submit_error', {
                    entity: entity,
                    response : response,
                    autosave : autosave
                });
    ...
    get(projeto: any, identifier : string) {
                
        return this.$q((resolve: any, reject: any) => {
            this.$http
            .get(this.nsjRouting.generate('gp_projetosescopo_get', { 'projeto': projeto,  'id': identifier }, true))
            .then((response: any) => {
                this.$rootScope.$broadcast('gp_projetosescopo_loaded', response.data);
                resolve(response.data);
            })
            .catch((response: any) => {
                reject(response);
