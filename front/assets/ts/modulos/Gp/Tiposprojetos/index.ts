...
export class GpTiposprojetosListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'tipoprojeto') {
        ...
        this.fields = $rootScope.FIELDS_GpTiposprojetos;
                $scope.$on('gp_tiposprojetos_deleted', (event: any) => {
        ...
        $rootScope.$on('gp_tiposprojetos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
