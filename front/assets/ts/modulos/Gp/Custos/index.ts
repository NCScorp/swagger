...
export class GpCustosListController {
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'custo') {
        ...
        this.fields = $rootScope.FIELDS_GpCustos;
                $scope.$on('gp_custos_deleted', (event: any) => {
        ...
        $rootScope.$on('gp_custos_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
