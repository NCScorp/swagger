...
export class GpTarefasListController {
    static $inject = [                                                                                        'utilService', '$scope', '$stateParams', '$state', 'GpTarefas', 'toaster'
    ,
    ...
    ,
        ...
        for (let i in $stateParams) {
            if (  [].indexOf(i) > -1 && $stateParams[i] !== undefined) {
                entityService.filters[i] = $stateParams[i];
                        } else if (typeof $stateParams[i] !== 'undefined' && typeof $stateParams[i] !== 'function' && i !== 'q' && i !== 'tarefa') {
        ...
        this.fields = $rootScope.FIELDS_GpTarefas;
                $scope.$on('gp_tarefas_deleted', (event: any) => {
        ...
        $rootScope.$on('gp_tarefas_submitted', (event: any, args: any) => {
            if (!args.autosave) {
                this.entityService.reload();
            }
        });
