...
export abstract class ServiceBase<EntityRequest> {
  /**
  ...
  /**
   * Variável que indica que está consumindo o backend do ErpApi
   */
  ...
  public getModuloEntidade(): string {
    return this.moduloEntidade;
  }

  public getNomeEntidade(): string {
  ...
  protected gerarUrl(nome: string, params: any): string {
    ...
    const url = this.nsjRouting.generate(
      nome,
      angular.extend(parametrosDestino, params),
      true, this.rotaErpApi
    );
    return url;
  }
   ...
   * @param tipo
   */
  public buscarConfiguracaoRequisicaoPorTipo(
   ...
   * @param tipo
   */
  public buscarConfiguracaoRequisicaoPorTipoCustomizado(
    tipoCustomizado: any
  ): IConfigRequisicao {
    return this.arrConfigRequisicoes.find((configFindIndex) => {
      return (
        configFindIndex.tipoCustomizado == tipoCustomizado &&
  ...
  public customPost<R>(
    tipoCustomizado: any,
    entidade: EntityRequest,
    construtores: any = {},
    params: any = {}
  ): Promise<R> {
    // Buscco a configuração da requisição
    const config =
      this.buscarConfiguracaoRequisicaoPorTipoCustomizado(tipoCustomizado);
   ...
   * @param construtores
   */
  // public criar(entidade: EntityType, construtores: any = {}): Promise<EntityType> {
  //     // Buscco a configuração da requisição
  ...
  public excluir(
    ...
    // Buscco a configuração da requisição
    const config = this.buscarConfiguracaoRequisicaoPorTipo(
      EConfigRequisicaoTipo.crtExcluir
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      // Disparo requisição
      this.$http
        .delete(url, { timeout: this.loading_deferred.promise })
        .then((response: any) => {
          resolve();
  ...
  reload<EntityResponse>() {
    ...
    return this.load() as EntityResponse[];
  }
    ...
    if (typeof filter !== 'undefined' && filter != null) {
      ...
      for (let fil in filter.filters) {
        if (typeof filter.filters[fil] !== 'object') {
          ...
          typeof filter.filters[fil] === 'object' &&
          filter.filters[fil].hasOwnProperty('startDate')
        ) {
          filters[fil] = {
          ...
          for (let pos in filter.filters[fil]) {
            if (
              filter.filters[fil][pos] === undefined ||
              (typeof filter.filters[fil][pos] === 'object' &&
                filter.filters[fil][pos].value === undefined) ||
              (typeof filter.filters[fil][pos] === 'object' &&
                filter.filters[fil][pos].value === null)
            ) {
          ...
          typeof filter.filters[fil] === 'object' &&
          filter.filters[fil].value !== undefined &&
          filter.filters[fil].value !== null
        ) {
          filters[fil] = filter.filters[fil];
        }
      }
  ...
  _load<EntityResponse>(
    constructors: any,
    offset: any,
    search: string,
    filters?: any,
    useFilterPromise?: boolean
  ) {
    // Busco configuração de rotas
    const config = this.buscarConfiguracaoRequisicaoPorTipo(
      EConfigRequisicaoTipo.crtListar
    );

    this.loading_deferred = this.$q.defer();
    this.loading_deferred_filter = this.$q.defer();
    return this.$q<EntityResponse[]>((resolve: any, reject: any) => {
      this.$http<EntityResponse[]>({
        method: 'GET',
        url: this.nsjRouting.generate(
          ...
          angular.extend(
            {},
            constructors,
            { offset: offset, filter: search },
            filters ? filters : this.filters
          ),
          true, this.rotaErpApi
        ),
        timeout: useFilterPromise
          ? this.loading_deferred_filter.promise
          : this.loading_deferred.promise,
      })
        .then((response) => {
          resolve(response.data);
        })
  ...
  load<EntityResponse>() {
    ...
    if (
      !this.loadParams.busy &&
      !this.loadParams.finished &&
      this.loadParams.to_load > 0
    ) {
      this.loadParams.busy = true;

      this._load<EntityResponse>(this.constructors, this.after, this.filter)
        .then((data) => {
            try {
              if (data.length > 0) {
                ...
                if (this.after) {
                  this.after[this.campoid] =
                  this.entities[this.entities.length - 1][this.campoid];
                  this.after[this.campoid] =
                  this.entities[this.entities.length - 1][this.campoid];
                }
              }
              if (data.length < 20) {
                ...
                this.dispararEvento(`${this.moduloEntidade}_${this.nomeEntidade}_partially_listed`, this.entities);
                this.loadParams.finished = false;
              }
              this.loaded = true;
    ...
    return this.entities as EntityResponse[];
  }
  ...
  */
  public getAll(filters: any = {}, construtores: any = {}): Promise<E[]> {
    // Busco a configuração da requisição
  ...
  deleteBulk(entities: EntityRequest[], force = true) {
    const entitiesDelete = entities.map((entity) => {
      return this.delete(entity[this.campoid], force).then((response) => response).catch((error) => error);
    });
    return Promise.allSettled(entitiesDelete);
  }
      ...
      if (force || confirm('Tem certeza que deseja deletar?')) {
        ...
        const request = this.$http.delete(
            this.nsjRouting.generate(
              config.nomeRota,
              angular.extend(this.constructors, { id: $identifier }),
              true, this.rotaErpApi
            ),
            { timeout: this.loading_deferred.promise }
          );
  ...
  _save<EntityResponse>(entity: EntityRequest, autosave: boolean = false) {
    let method, url: string;
    let config: IConfigRequisicao = null;
    if (entity[this.campoid]) {
      config = this.buscarConfiguracaoRequisicaoPorTipo(
        EConfigRequisicaoTipo.crtAtualizar
      );
      method = 'PUT';
      ...
      config = this.buscarConfiguracaoRequisicaoPorTipo(
        EConfigRequisicaoTipo.crtCriar
      );
      method = 'POST';
    ...
    this.loading_deferred = this.$q.defer();
    return this.$http<EntityResponse>({
      method: method,
      url: url,
      data: data,
      timeout: this.loading_deferred.promise,
  ...
  save<EntityResponse>(entity: EntityRequest, autosave: boolean = false) {
    ...
    this._save<EntityResponse>(entity, autosave)
      .then((response) => {
  ...
  get<EntityResponse>(identifier?: Record<string, string> | object) {
    const config = this.buscarConfiguracaoRequisicaoPorTipo(
      EConfigRequisicaoTipo.crtBuscar
    );
    ...
    return this.$q((resolve, reject) => {
      this.$http
        .get<EntityResponse>(
          this.nsjRouting.generate(config.nomeRota, {
              ...this.constructors,
             ...parameters,
            }, true, this.rotaErpApi),
          {
  ...
  saveBulk<EntityResponse>(
    entities: EntityRequest[],
    autosave: boolean = false
  ) {
    const entityPromise = entities.map((entity) => {
      return this._save<EntityResponse>(entity, autosave).then((response) => response).catch((error) => error);
    }
    );

    return Promise.allSettled(entityPromise);
  }
